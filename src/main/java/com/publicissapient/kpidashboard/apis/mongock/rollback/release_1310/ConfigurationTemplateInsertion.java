...
public class ConfigurationTemplateInsertion {
	...
	public ConfigurationTemplateInsertion(MongoTemplate mongoTemplate) {
		...
		ConfigurationTemplateDataFactory configurationTemplateDataFactory =
				ConfigurationTemplateDataFactory.newInstance();
	...
	public void rollback() {
		...
		List<Document> insertedTemplates =
				mongoTemplate
						.getCollection(CONFIGURATION_TEMPLATE)
						.find(new Document(TEMPLATE_CODE, new Document("$in", Arrays.asList("1", "2", "3"))))
						.into(new ArrayList<>());
