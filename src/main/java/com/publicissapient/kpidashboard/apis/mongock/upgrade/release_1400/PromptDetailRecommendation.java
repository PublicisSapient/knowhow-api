...
@ChangeUnit(id = "recommendation_update", order = "14000", author = "shi6", systemVersion = "14.0.0")
public class PromptDetailRecommendation {
	...
	private void kpirecommendationUpdate() {
		...
		update.append("$set", new Document().append("key", "kpi-recommendation").append(CONTEXT,
	...
	private void anaylysisReportUpdate() {
		...
		Document update = new Document();
		update.append("$set", new Document().append("key", "kpi-correlation-analysis-report").append(CONTEXT,
	...
	public void rollback() {
		MongoCollection<Document> collection = mongoTemplate.getCollection(PROMPT_DETAILS_COLLECTION);
		collection.deleteMany(new Document("key", new Document("$in", Arrays.asList(KPI_CORRELATION_ANALYSIS_REPORT,
		...
		collection.insertMany(
				Arrays.asList(kpiCorrelationAnalysis, kpiRecommendationPrompt));
