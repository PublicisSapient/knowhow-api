 ...
 */
@ChangeUnit(id = "epic_hygiene_slingshot_kpi_insert", order = "17175", author = "kunkambl", systemVersion = "17.1.0")
public class EpicHygieneSlingshotChangeUnit {
	...
	public void insertKpiMaster(MongoTemplate mongoTemplate) {
		Document kpiMaster = new Document().append(KPI_ID_FIELD, KPI_ID).append("kpiName", "Epic Hygiene")
				.append("isDeleted", "False").append("defaultOrder", 2).append("kpiCategory", "Slingshot")
				.append("kpiSubCategory", "Sandbox").append("kpiUnit", "%").append("chartType", "")
				.append("xAxisLabel", "").append("yAxisLabel", "Percentage").append("showTrend", false)
				.append("isPositiveTrend", true).append("calculateMaturity", true)
				.append("maturityRange", Arrays.asList("0-20", "20-40", "40-60", "60-80", "80-"))
				.append("hideOverallFilter", true).append("kpiSource", "Jira").append("maxValue", 100)
				.append("thresholdValue", 70.0).append("upperThresholdBG", "white").append("lowerThresholdBG", "red")
				.append("kanban", false).append("groupId", 312)
				...
				.append("isAdditionalFilterSupport", false).append("combinedKpiSource", "Jira/Azure Boards/Rally");

		mongoTemplate.getCollection(KPI_MASTER_COLLECTION).replaceOne(new Document(KPI_ID_FIELD, KPI_ID), kpiMaster,
	...
	public void insertKpiColumnConfig(MongoTemplate mongoTemplate) {
		Document columnConfig = new Document().append("basicProjectConfigId", null).append(KPI_ID_FIELD, KPI_ID)
				.append("kpiColumnDetails",
						Arrays.asList(column("Epic ID", 1), column("Epic Name", 2), column("Status", 3),
								column("Assignee", 4), column("Hygiene Score", 5), column("Overall Status", 6),
								column("Recommendations", 7)));

		mongoTemplate.getCollection(KPI_COLUMN_CONFIGS_COLLECTION).replaceOne(new Document(KPI_ID_FIELD, KPI_ID),
				columnConfig, new ReplaceOptions().upsert(true));
	}
	...
	public void insertFieldMappingStructure(MongoTemplate mongoTemplate) {
		Document thresholdStructure = new Document().append(FIELD_NAME, THRESHOLD_FIELD)
				.append("fieldLabel", "Target KPI Value").append("fieldType", "number")
				.append("section", "Project Level Threshold").append("processorCommon", false)
				...
				.append("fieldDisplayOrder", 1).append("sectionOrder", 6).append("mandatory", false)
				.append("nodeSpecific", false);

		Document readinessDimensionsStructure = new Document().append(FIELD_NAME, JIRA_FIELDS_SELECTION_FIELD)
				.append("fieldLabel", READINESS_DIMENSIONS_LABEL).append("placeHolderText", READINESS_DIMENSIONS_LABEL)
				.append("fieldType", "chips").append("section", "Custom Fields Mapping").append("fieldCategory", "fields")
				...
				.append("filterGroup", List.of("CustomField")).append("nodeSpecific", false).append("fieldDisplayOrder", 4)
				.append("toggleLabelLeft", null).append("toggleLabelRight", null).append("sectionOrder", 3)
				.append("mandatory", false).append("readOnly", null);

		mongoTemplate.getCollection(FIELD_MAPPING_STRUCTURE_COLLECTION).replaceOne(
				new Document(FIELD_NAME, THRESHOLD_FIELD), thresholdStructure, new ReplaceOptions().upsert(true));
		mongoTemplate.getCollection(FIELD_MAPPING_STRUCTURE_COLLECTION).replaceOne(
				new Document(FIELD_NAME, JIRA_FIELDS_SELECTION_FIELD), readinessDimensionsStructure,
				new ReplaceOptions().upsert(true));
	}
	...
	public void rollback(MongoTemplate mongoTemplate) {
		...
		mongoTemplate.getCollection(FIELD_MAPPING_STRUCTURE_COLLECTION).deleteMany(new Document(FIELD_NAME,
	...
	private static Document column(String columnName, int order) {
		return new Document().append(COLUMN_NAME, columnName).append(ORDER, order).append(IS_SHOWN, true)
