...
@ChangeUnit(id = "epic_hygiene_prompt_details", order = "17177", author = "kunkambl", systemVersion = "17.1.0")
public class EpicHygienePromptDetailsChangeUnit {
	...
	private void insertEpicHygienePromptDetails() {
		...
		String context = "You are an Expert Epic Readiness Analyzer Agent. Your job is to assess how ready each Jira "
		...
		String input = "=== Readiness Dimensions ===\n"
		...
		String outputFormat = "Return a JSON ARRAY - one element per input Epic, in the same order as the input. No "
		...
		Update update = new Update().set(KEY, EPIC_HYGIENE_PROMPT).set(CONTEXT, context)
				.set(TASK,
				...
				.set(INPUT, input).set(OUTPUT_FORMAT, outputFormat)
				.set(PLACEHOLDERS, Arrays.asList(HYGIENE_RULES_PLACEHOLDER, JIRA_ISSUES_PLACEHOLDER));

		mongoTemplate.upsert(new Query(Criteria.where(KEY).is(EPIC_HYGIENE_PROMPT)), update, PROMPT_DETAILS_COLLECTION);
	...
	private void deleteEpicHygienePromptDetails() {
		...
		mongoTemplate.remove(new Query(Criteria.where(KEY).is(EPIC_HYGIENE_PROMPT)), PROMPT_DETAILS_COLLECTION);
