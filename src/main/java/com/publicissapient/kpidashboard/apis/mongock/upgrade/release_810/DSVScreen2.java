...
public class DSVScreen2 {
		...
		fieldMappingStructure.insertMany(
				Arrays.asList(
						...
						createDocument(
								...
								"Status to identify QA completed issues",
								"workflow",
								"WorkFlow Status Mapping",
						...
						createDocument(
								...
								"Status when issue type is put on Hold",
								"workflow",
								"WorkFlow Status Mapping",
								"All status that identify hold/blocked statuses.")));
	}
	...
	private void updateMetadata() {
		...
		metaDataOperations.add(
				new UpdateManyModel<>(
						Filters.or(Filters.eq("templateCode", "8"), Filters.eq("tool", "Azure")),
						Updates.addToSet(
								"workflow",
								new Document("type", "firstDevstatus")
										.append(
												"value",
												Arrays.asList(
														"In Analysis", "IN ANALYSIS", "In Development", "In Progress")))));

		metaDataOperations.add(
				new UpdateManyModel<>(
						Filters.eq("templateCode", "7"),
						Updates.addToSet(
								"workflow",
								new Document("type", "jiraStatusForInProgressKPI154")
										.append(
												"value", Arrays.asList("In Analysis", "In Development", "In Progress")))));
		...
		mongoTemplate
				...
				.bulkWrite(metaDataOperations, new BulkWriteOptions().ordered(false));
	}
