 ...
 */
@ChangeUnit(
		id = "r_index_name_changes_document_db",
		order = "09003",
		author = "pawkandp",
		systemVersion = "9.0.0")
public class CreateDatabaseIndexesChangeLog {
	...
	public void clearAndExecuteJiraIssueIndexes() {
		...
		// Index 1
		mongoTemplate
				.getCollection(JIRA_ISSUE)
				.createIndex(new Document("_id", 1), new IndexOptions().name("_id_"));
		...
		mongoTemplate
				.getCollection(JIRA_ISSUE)
				.createIndex(
						new Document(BASIC_PROJECT_CONFIG_ID, 1)
								.append(DEFECT_STORY_ID, 1)
								.append(TYPE_NAME, 1),
		...
		mongoTemplate
				.getCollection(JIRA_ISSUE)
				.createIndex(
						new Document(TYPE_NAME, 1).append(DEFECT_STORY_ID, 1),
	...
	public void clearAndExecuteJiraIssueCustomHistoryIndexes() {
		...
		// Index 1
		mongoTemplate
				.getCollection(JIRA_ISSUE_CUSTOM_HISTORY)
				.createIndex(
		...
		mongoTemplate
				.getCollection(JIRA_ISSUE_CUSTOM_HISTORY)
				.createIndex(new Document("storyID", 1), new IndexOptions().name("storyID_1"));
	...
	public void clearAndExecuteKanbanJiraIssueIndexes() {
		...
		mongoTemplate
				.getCollection(KANBAN_JIRA_ISSUE)
				.createIndex(
						new Document(PROJECT_ID, 1).append(STATUS, 1),
		...
		mongoTemplate
				.getCollection(KANBAN_JIRA_ISSUE)
				.createIndex(
						new Document(PROJECT_ID, 1).append(TYPE_NAME, 1),
		...
		mongoTemplate
				.getCollection(KANBAN_JIRA_ISSUE)
				.createIndex(
						new Document(PROJECT_ID, 1).append(CREATED_DATE, -1),
		...
		mongoTemplate
				.getCollection(KANBAN_JIRA_ISSUE)
				.createIndex(
						new Document("processorId", 1)
								.append(BASIC_PROJECT_CONFIG_ID, 1)
								.append("changeDate", -1),
	...
	public void clearAndExecuteKanbanIssueCustomHistoryIndexes() {
		...
		// Index 1
		mongoTemplate
				.getCollection(KANBAN_JIRA_ISSUE_CUSTOM_HISTORY)
				.createIndex(
						new Document(BASIC_PROJECT_CONFIG_ID, 1),
						new IndexOptions().name(BASIC_PROJECT_CONFIG_ID_1));
	...
	public void clearAndExecuteSprintDetailsIndexes() {
		...
		mongoTemplate
				.getCollection(SPRINT_DETAILS)
				.createIndex(
						new Document(SPRINT_ID, -1), new IndexOptions().name("sprintID_-1").unique(true));

		// Index 2
		mongoTemplate
				.getCollection(SPRINT_DETAILS)
				.createIndex(new Document(SPRINT_ID, 1), new IndexOptions().name("sprintID_1"));
	...
	public void clearAndExecuteTestCaseDetailsIndexes() {
		...
		// Index 1
		mongoTemplate
				.getCollection(TEST_CASE_DETAILS)
				.createIndex(
						new Document(BASIC_PROJECT_CONFIG_ID, 1)
								.append("isTestCanBeAutomated", -1)
								.append(TYPE_NAME, 1),
	...
	public void clearAndExecuteMergeRequestsIndexes() {
		...
		mongoTemplate
				.getCollection(MERGE_REQUESTS)
				.createIndex(
						new Document(PROCESSOR_ITEM_ID, 1), new IndexOptions().name("processorItemId_1"));
		...
		mongoTemplate
				.getCollection(MERGE_REQUESTS)
				.createIndex(
						new Document(PROCESSOR_ITEM_ID, 1)
								.append(CREATED_DATE, 1)
								.append("fromBranch", 1)
								.append("closedDate", 1),
	...
	public void clearAndExecuteProcessorItemsIndexes() {
		...
		mongoTemplate
				.getCollection(PROCESSOR_ITEMS)
				.createIndex(new Document("_id", 1), new IndexOptions().name("_id_"));

		// Create index for "toolConfigId" field in the collection
		mongoTemplate
				.getCollection(PROCESSOR_ITEMS)
				.createIndex(new Document("toolConfigId", 1), new IndexOptions().name("toolConfigId_1"));
	}
