...
public class DefectsBreachedSLAsChangeUnit {
	...
	public void execution() {
		...
		mongoTemplate.getCollection("field_mapping_structure")
				.deleteMany(new Document(FIELD_NAME,
						new Document("$in",
								List.of("includedSeveritySlasKPI195", "excludedDefectPrioritiesKPI195",
										"includedDefectRootCausesKPI195", "excludedDefectRejectionStatusesKPI195",
										"excludedDefectResolutionTypesKPI195", "includedDefectClosureStatusesKPI195",
	...
	private Document createColumnDetail(String columnName, int order) {
		return new Document().append("columnName", columnName).append("order", order).append("isShown", true)
				.append("isDefault", true);
	}

	private Document constructKpiColumnConfigDocument() {
		return new Document().append("basicProjectConfigId", null).append(KPI_ID, KPI_195).append("kpiColumnDetails",
				List.of(createColumnDetail("Sprint Name", 0), createColumnDetail("Defect ID", 1),
						createColumnDetail("Defect Description", 2), createColumnDetail("Story ID", 3),
						createColumnDetail("Story Description", 4), createColumnDetail("Squad", 5),
						createColumnDetail("Root Cause", 6), createColumnDetail("Defect Priority", 7),
						createColumnDetail("Defect Removed", 8), createColumnDetail("Defect Status", 9),
						createColumnDetail("Time Spent (in hours)", 10), createColumnDetail("Resolution Time", 11),
						createColumnDetail("Defect SLA", 12), createColumnDetail("SLA Breached", 12)));
	}

	private List<Document> constructFieldMappingStructureDocuments() {
		List<Document> fieldMappingStructureDocuments = new ArrayList<>();

		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "includedSeveritySlasKPI195")
				.append(FIELD_LABEL, "Defect SLA").append(FIELD_DISPLAY_ORDER, 1)
				.append(FIELD_TYPE, "multiselect-dropdown").append(FIELD_CATEGORY, "fields")
				.append(SECTION, "Custom Fields Mapping").append(SECTION_ORDER, 1).append(MANDATORY, true)
				.append(TOOLTIP, new Document().append(DEFINITION,
						"The agreed-upon time limit within which a defect must be resolved, as defined by service-level agreements (SLAs)"))
				.append(OPTIONS, List.of(
						new Document().append(LABEL, "s1").append(STRUCTURED_VALUE,
								new Document().append(SEVERITY, "s1").append(SLA, 24).append(TIMEUNIT, "Hours")),
						new Document().append(LABEL, "s2").append(STRUCTURED_VALUE,
								new Document().append(SEVERITY, "s2").append(SLA, 48).append(TIMEUNIT, "Hours")),
						new Document().append(LABEL, "s3").append(STRUCTURED_VALUE,
								new Document().append(SEVERITY, "s3").append(SLA, 5).append(TIMEUNIT, "Days")),
						new Document().append(LABEL, "s4").append(STRUCTURED_VALUE,
								new Document().append(SEVERITY, "s4").append(SLA, 10).append(TIMEUNIT, "Days")))));

		// Priority to be excluded
		fieldMappingStructureDocuments
				.add(new Document().append(FIELD_NAME, "excludedDefectPrioritiesKPI195")
		...
		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "includedDefectRootCausesKPI195")
		...
		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "excludedDefectRejectionStatusesKPI195")
				...
				.append(FIELD_CATEGORY, "workflow").append(SECTION, WORKFLOW_STATUS_MAPPING)
				.append(FIELD_DISPLAY_ORDER, 1).append(SECTION_ORDER, 3)
				.append(TOOLTIP, new Document().append(DEFINITION, "All workflow statuses used to reject defects.")));

		// Resolution type to be excluded
		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "excludedDefectResolutionTypesKPI195")
		...
		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "includedDefectClosureStatusesKPI195")
				.append(FIELD_LABEL, "Status Consider for Issue Closure").append(FIELD_TYPE, CHIPS)
				.append(FIELD_CATEGORY, "workflow").append(SECTION, WORKFLOW_STATUS_MAPPING)
				.append(FIELD_DISPLAY_ORDER, 3).append(SECTION_ORDER, 4)
				.append(TOOLTIP, new Document().append(DEFINITION,
						"Status considered for defect closure (Mention completed status of all types of defects)"))
				.append(MANDATORY, true));

		// Target KPI Value
		fieldMappingStructureDocuments.add(new Document().append(FIELD_NAME, "thresholdValueKPI195")
				.append(FIELD_LABEL, "Target KPI Value").append(FIELD_TYPE, "number")
				.append(SECTION, "Project Level Threshold").append(PROCESSOR_COMMON, false)
				.append(TOOLTIP, new Document().append(DEFINITION,
						...
						.append(FIELD_DISPLAY_ORDER, 1).append(SECTION_ORDER, 4).append(MANDATORY, false)
	...
	private Document constructKpiCategoryMappingDocument() {
		return new Document().append(KPI_ID, KPI_195).append("categoryId", "quality").append("kpiOrder", 17)
				.append(KANBAN, false);
	}

	private Document constructKpiMasterDocument() {
		return new Document().append(KPI_ID, KPI_195).append("kpiName", "Defects Breached SLAs")
				.append("isDeleted", "False").append("defaultOrder", 28).append("kpiUnit", "%")
				.append("chartType", "stacked-bar-chart").append("upperThresholdBG", "red")
				.append("lowerThresholdBG", "white").append("xAxisLabel", "Sprints")
				.append("yAxisLabel", "% defects that breached SLA").append("showTrend", true)
				.append("isPositiveTrend", false).append("calculateMaturity", false).append("hideOverallFilter", false)
				.append("kpiSource", "Jira").append("maxValue", 100).append("thresholdValue", 20).append(KANBAN, false)
				.append("groupId", 26)
				.append("kpiInfo", new Document().append(DEFINITION,
						...
						.append("formula",
								List.of(new Document().append("lhs", "Defects Breached SLA (%)")
										.append("operator", "division").append("operands",
												List.of("Number of Resolved Defects that breached" + " SLA",
						...
						.append("details", List.of(new Document().append("type", "link").append("kpiLinkDetail",
								new Document().append("text", "Detailed Information at").append("link",
										"https://knowhow.suite.publicissapient.com/wiki/spaces/PS/pages/270794755/Defects+Breached+SLAs")))))
				.append("aggregationCriteria", "average").append("isTrendCalculative", false)
				.append("isAdditionalFilterSupport", true).append("combinedKpiSource", "Jira");
	}
}
