...
@ChangeUnit(
		id = "r_jira_configuration_type_migration",
		order = "013102",
		author = "girpatha",
...
public class JiraConfigurationTypeChangeUnit {
	...
	public void rollback() {
		// Recreate the original state by setting jiraConfigurationType based on
		// conditions
		List<Document> jiraConfigs =
				mongoTemplate
		...
		for (Document config : jiraConfigs) {
			...
			Document update =
					new Document("$set", new Document(JIRA_CONFIGURATION_TYPE, jiraConfigurationType));
