...
public class ProjectOrganizationStructureChangeLog {
	...
	public void initializeHierarchyLevels() {
		if (mongoTemplate.getCollection(HIERARCHY_LEVELS).countDocuments() == 0) {
			List<Document> hierarchyLevels =
					Arrays.asList(
							new Document(LEVEL, 1)
									.append(HIERARCHY_LEVEL_ID, "hierarchyLevelOne")
									.append(HIERARCHY_LEVEL_NAME, "Organization"),
							new Document(LEVEL, 2)
									.append(HIERARCHY_LEVEL_ID, "hierarchyLevelTwo")
									.append(HIERARCHY_LEVEL_NAME, "Business Unit"),
							new Document(LEVEL, 3)
									.append(HIERARCHY_LEVEL_ID, "hierarchyLevelThree")
	...
	public void initializeHierarchyLevelSuggestions() {
		if (mongoTemplate.getCollection(HIERARCHY_LEVEL_SUGGESTIONS).countDocuments() == 0) {
			List<Document> levelSuggestions =
					Arrays.asList(
							new Document(HIERARCHY_LEVEL_ID, "hierarchyLevelOne")
									.append(VALUES, Collections.singletonList("Organization")),
							new Document(HIERARCHY_LEVEL_ID, "hierarchyLevelTwo")
									.append(VALUES, Arrays.asList("Business Unit 1", "Business Unit 2")),
							new Document(HIERARCHY_LEVEL_ID, "hierarchyLevelThree")
									.append(VALUES, Arrays.asList("Portfolio 1", "Portfolio 2")));
