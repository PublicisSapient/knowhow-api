...
@ChangeUnit(
		id = "repo_tool_provider_url_change",
		order = "8110",
		author = "kunkambl",
		systemVersion = "8.1.0")
	...
	public void changeRepoToolProviderTestApiUrls() {
		...
		Document updateGl =
				new Document(
						"$set",
						new Document()
								.append("testApiUrl", "/api/v4/projects/")
								.append("repoToolProvider", "gitlab"));
	...
	public void updateThresholds() {
		mongoTemplate
				...
				.updateMany(
						...
						new Document(
								"$set",
								new Document("upperThresholdBG", "red").append("lowerThresholdBG", "white")));
	}

	public void updateKpi162() {
		Document filter = new Document("kpiId", "kpi162");
		Document update =
				new Document("$set", new Document("calculateMaturity", false).append("showTrend", false));
	...
	public void changeRepoToolProviderTestApiUrlsRollback() {
		...
		Document update =
				new Document(
						"$set",
						new Document()
								.append("testApiUrl", "https://gitlab.com/api/v4/projects/")
								.append("repoToolProvider", ""));
	...
	public void updateThresholdsRollback() {
		mongoTemplate
				...
				.updateMany(
						new Document("kpiId", new Document("$in", Arrays.asList("kpi160", "kpi158"))),
						new Document(
								"$set", new Document("upperThresholdBG", "").append("lowerThresholdBG", "")));
	}

	public void updateKpi162Rollback() {
		Document filter = new Document("kpiId", "kpi162");
		Document update =
				new Document("$set", new Document("calculateMaturity", true).append("showTrend", true));
		mongoTemplate.getCollection("kpi_master").updateOne(filter, update);
	}
}
