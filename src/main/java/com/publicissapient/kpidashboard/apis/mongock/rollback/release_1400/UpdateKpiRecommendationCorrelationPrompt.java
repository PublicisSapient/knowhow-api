...
@ChangeUnit(
		id = "r_update_kpi_recommendation_correlation_prompt",
		order = "014006",
		author = "kunkambl",
...
public class UpdateKpiRecommendationCorrelationPrompt {
	...
	private static final String OUTPUT_FORMAT_UPDATE =
	...
	private static final String OUTPUT_FORMAT_ROLLBACK =
	...
	public void execute() {
		Document updateFields =
				new Document("$set", new Document(OUTPUT_FORMAT_KEY, OUTPUT_FORMAT_ROLLBACK));
		mongoTemplate
				.getCollection("prompt_details")
				.updateOne(new Document(KEY, KEY_VALUE), updateFields);
	}

	@RollbackExecution
	public void rollback() {
		Document updateFields =
				new Document("$set", new Document(OUTPUT_FORMAT_KEY, OUTPUT_FORMAT_UPDATE));
		mongoTemplate
				.getCollection("prompt_details")
				.updateOne(new Document(KEY, KEY_VALUE), updateFields);
	}
