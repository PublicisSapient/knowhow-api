...
/**
 * Change unit to insert a new field mapping & kpi master for new defect reopen
 * rate KPI
 *
 * @author shunaray
 */
...
public class DefectReopenChangeUnit {
	...
	private void insertFieldMapping() {
		List<Document> fieldMappings = List.of(createFieldMapping(Map.of(FIELD_NAME, "resolutionTypeForRejectionKPI190",
				FIELD_LABEL, "Resolution type to be excluded", FIELD_TYPE, "chips", SECTION, WORK_FLOW_STATUS_MAPPING,
				TOOLTIP, "Resolution types for defects that can be excluded from Defect reopen rate calculation",
				FIELD_DISPLAY_ORDER, 1, SECTION_ORDER, 1, MANDATORY, false)),
				createFieldMapping(Map.of(FIELD_NAME, "jiraDefectRejectionStatusKPI190", FIELD_LABEL,
				...
				createFieldMapping(Map.of(FIELD_NAME, "thresholdValueKPI190", FIELD_LABEL, "Target KPI Value",
						...
						FIELD_DISPLAY_ORDER, 1, SECTION_ORDER, 2, MANDATORY, false)));
	...
	private static Document insertKPIMaster() {
		return new Document().append(KPI_ID, KPI_190).append("kpiName", "Defect Reopen Rate")
				.append("isDeleted", "False").append("defaultOrder", 8).append("kpiUnit", "Number")
				...
				.append("kpiInfo", new Document().append("definition",
						...
						.append("formula",
								List.of(new Document().append("lhs", "Defect Reopen Rate (%)")
										.append("operator", "division")
										.append("operands", List.of("Reopened Defects", "Total Resolved Defects"))))
				...
				.append("isTrendCalculative", false).append("isAdditionalFilterSupport", true)
				.append("combinedKpiSource", "Jira/Azure");
	}

	private Document createFieldMapping(Map<String, Object> fieldData) {
		Document document = new Document();
		document.append(FIELD_NAME, fieldData.get(FIELD_NAME)).append(FIELD_LABEL, fieldData.get(FIELD_LABEL))
				.append(FIELD_TYPE, fieldData.get(FIELD_TYPE)).append(SECTION, fieldData.get(SECTION))
				.append("processorCommon", false).append(TOOLTIP, new Document("definition", fieldData.get(TOOLTIP)))
				.append(FIELD_DISPLAY_ORDER, fieldData.get(FIELD_DISPLAY_ORDER))
				.append(SECTION_ORDER, fieldData.get(SECTION_ORDER)).append(MANDATORY, fieldData.get(MANDATORY))
