...
public class ReportService {
	...
	public ReportService(
			ReportRepository reportRepository, AuthenticationService authenticationService) {
	...
	public ServiceResponse create(ReportRequest reportRequest) {
		...
		if (existingByNameAndCreatedBy.isPresent()) {
			log.warn(
					"Report with the same name already exists for user: {}", CommonUtils.sanitize(createdBy));
			throw new DuplicateReportException(
					"Report with the same name already exists for user: " + createdBy);
		}
		...
		if (existingReport.isPresent()) {
			log.warn(
					"Report already exists with ID: {}",
					CommonUtils.sanitize(String.valueOf(existingReport.get().getId())));
			throw new DuplicateReportException(
					"Report already exists with ID: " + existingReport.get().getId());
		}
		...
		log.debug(
				...
				CommonUtils.sanitize(String.valueOf(report.getId())));
	 ...
	 */
	public ServiceResponse update(String id, ReportRequest reportRequest)
		...
		log.debug(
				"Report updated successfully with ID: {}",
				CommonUtils.sanitize(String.valueOf(existingReport.getId())));
	...
	private Optional<Report> findExistingReport(ReportRequest reportRequest) {
		...
		final ModelMapper modelMapper = new ModelMapper();
		List<KPI> kpis =
				reportRequest.getKpis().stream().map(kpiDTO -> modelMapper.map(kpiDTO, KPI.class)).toList();
	...
	public ServiceResponse getReportsByCreatedBy(String createdBy, int page, int size) {
		log.info(
				"Fetching reports by createdBy: {}, page: {}, size: {}",
		...
		if (reportsPage.hasContent()) {
			Page<ReportRequest> reportRequestPage =
					reportsPage.map(report -> modelMapper.map(report, ReportRequest.class));
