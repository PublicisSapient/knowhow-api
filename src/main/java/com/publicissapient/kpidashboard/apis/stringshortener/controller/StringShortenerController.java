...
public class StringShortenerController {
	...
	@Operation(
			summary = "Create Short String",
			description = "API to create a short string for given long string")
	@ApiResponses(
			value = {
				@ApiResponse(responseCode = "200", description = "Short string created successfully"),
				@ApiResponse(responseCode = "404", description = "Failed to create short string")
			})
	@PostMapping("/shorten")
	public ResponseEntity<ServiceResponse> createShortString(
			@Parameter(
							description = "StringShortenerDTO object containing long string details",
							required = true)
					@RequestBody
					StringShortenerDTO stringShortenerDTO) {
	...
	@ApiResponses(
			value = {
				@ApiResponse(responseCode = "200", description = "Long string fetched successfully"),
				@ApiResponse(responseCode = "404", description = "Failed to fetch long string")
			})
