...
public class CacheEventLogger implements CacheEventListener<Object, Object> {
	...
	public void onEvent(CacheEvent<? extends Object, ? extends Object> event) {
		...
		if (cacheName.contains("accountHierarchy")) {
			switch (event.getType()) {
				case CREATED:
					logger.info(
							"[CACHE-EVENT] {} - {} - CREATED - Key: {} - Cache now has data",
							timestamp,
							cacheName,
							key);
					break;
				...
				case UPDATED:
					logger.info(
							"[CACHE-EVENT] {} - {} - UPDATED - Key: {} - Cache data refreshed",
							timestamp,
							cacheName,
							key);
					break;
				...
				case EVICTED:
					logger.warn(
							"[CACHE-EVENT] {} - {} - EVICTED - Key: {} - CRITICAL: Cache entry removed due to memory pressure!",
							timestamp,
							cacheName,
							key);
					logMemoryInfo();
				...
				case EXPIRED:
					logger.warn(
							"[CACHE-EVENT] {} - {} - EXPIRED - Key: {} - Cache entry expired",
							timestamp,
							cacheName,
							key);
					break;
				...
				case REMOVED:
					logger.warn(
							"[CACHE-EVENT] {} - {} - REMOVED - Key: {} - Cache entry manually removed",
							timestamp,
							cacheName,
							key);
					break;
				...
				default:
					logger.info("[CACHE-EVENT] {} - {} - {} - Key: {}", timestamp, cacheName, eventType, key);
			}
		}
		...
		if (event.getType().toString().equals("EVICTED")) {
			logger.warn(
					"[CACHE-EVICTION] {} - Cache: {} - Key: {} - Evicted due to memory pressure",
					timestamp,
					cacheName,
					key);
		}
	...
	private void logMemoryInfo() {
		try {
			...
			logger.warn(
					"[MEMORY-INFO] Total: {}MB, Used: {}MB, Free: {}MB, Max: {}MB",
					totalMemory,
					usedMemory,
					freeMemory,
					maxMemory);
			...
			if (memoryUsagePercent > 80) {
				logger.error(
						"[MEMORY-ALERT] Memory usage is {}% - This may cause cache evictions!",
