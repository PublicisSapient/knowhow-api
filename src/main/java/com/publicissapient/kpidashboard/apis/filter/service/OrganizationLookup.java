 ...
 * that need to traverse organizational structures frequently.
 *
 * <p>The class builds three primary indexes during construction:
 *
 * <ul>
 *   <li><strong>Node ID Index:</strong> Direct access to entities by their unique identifiers
 *   <li><strong>Parent ID Index:</strong> Fast retrieval of child entities for any parent
 *   <li><strong>Level Index:</strong> Efficient access to all entities at a specific hierarchy
 *       level
 * </ul>
 */
@Slf4j
public class OrganizationLookup {
	...
	/**
	 * @param flatAccountFilteredData flat collection of the organization entities at which the
	 *     current user has access. This is sent through the constructor because this utility class
	 *     should be used to perform lookups based on a filtered data which is required for a specific
	 *     use case.
	 */
	public OrganizationLookup(Set<AccountFilteredData> flatAccountFilteredData) {
		...
		for (AccountFilteredData accountFilteredData : flatAccountFilteredData) {
			if (Objects.nonNull(accountFilteredData)
					&& StringUtils.isNotBlank(accountFilteredData.getNodeId())) {
	 ...
	 *
	 * <p>This method provides efficient access to entities grouped by their position in the
	 * organizational hierarchy. Common use cases include:
	 *
	 * <ul>
	 *   <li>Dashboard aggregations at specific organizational levels
	 *   <li>Level-based filtering and reporting
	 *   <li>Organizational structure analysis
	 * </ul>
	 *
	 * <p>The method returns an empty list if no entities exist at the specified level, making it safe
	 * to use in iterative operations without null checks.
	 *
	 ...
	 *
	 * <p>This method provides direct access to entities using their node ID, which is the primary key
	 * in the organizational hierarchy. The method may return multiple entities if the same node ID
	 * appears in different contexts (though this is rare in well-formed hierarchies).
	 *
	 ...
	 *
	 * <p>This method performs a hierarchical search to find all entities at the target level that are
	 * descendants of the specified parent. The search traverses the organizational tree recursively,
	 * making it suitable for finding entities that may be several levels below the parent.
	 *
	 * <p>Usage sample: Getting all projects under a specific account
	 *
	 * <p>If the parent node doesn't exist or has no descendants at the target level, an empty list is
	 * returned.
	 *
	 ...
	 *
	 * <p>This method efficiently processes multiple parent nodes simultaneously, returning a map
	 * where each key is a parent node ID and the value is the list of its descendants at the
	 * specified level. This bulk operation is more efficient than calling {@link
	 * #getChildrenByParentNodeId(String, int)} multiple times.
	 *
	 * <p>Parents without descendants at the target level will have empty lists as values.
	 *
	 ...
	 */
	public Map<String, List<AccountFilteredData>> getChildrenGroupedByParentNodeIds(
			Set<String> parentNodeIds, int childLevel) {
		List<AccountFilteredData> parents =
				parentNodeIds.stream()
						.flatMap(parentNodeId -> this.byNodeId.get(parentNodeId).stream())
						.toList();
	 ...
	 *
	 * <p>This method finds all entities at the parent level and then groups their descendants at the
	 * child level. It's a convenience method that combines level-based lookup with parent-child
	 * grouping, eliminating the need to first retrieve parent entities manually.
	 *
	 * <p>This is particularly useful for organizational reporting where you need to see the
	 * distribution of lower-level entities across all entities at a higher level. For example,
	 * grouping all projects by their parent accounts.
	 *
	 * <p><strong>Validation:</strong> The method validates that the child level is numerically higher
	 * than the parent level, as organizational hierarchies typically use ascending numbers for deeper
	 * levels.
	 *
	...
	public Map<String, List<AccountFilteredData>> getChildrenGroupedByParentNodeIds(
		...
		if (childLevel < parentLevel) {
			throw new IllegalArgumentException(
					String.format(
							"The child level must be higher than the parent level. Received child "
									+ "level: %s, received parent level %s",
	 ...
	 *
	 * <p>This private method implements the core recursive algorithm for traversing the
	 * organizational hierarchy. It performs a depth-first search starting from the current node and
	 * continues until it finds all entities at the target level.
	 *
	 * <p>The algorithm works as follows:
	 *
	 * <ol>
	 *   <li>If the current node is at the target level, add it to results
	 *   <li>Otherwise, recursively search all direct children
	 *   <li>Aggregate results from all recursive calls
	 * </ol>
	 *
	 * <p><strong>Performance Note:</strong> The time complexity is O(d) where d is the total number
	 * of descendants that need to be examined. In well-balanced hierarchies, this is typically much
	 * smaller than the total dataset size.
	 *
	 ...
	 */
	private List<AccountFilteredData> findDescendantsByLevel(
			AccountFilteredData currentNode, int targetLevel) {
		...
		List<AccountFilteredData> children =
				this.byParentId.getOrDefault(currentNode.getNodeId(), List.of());
