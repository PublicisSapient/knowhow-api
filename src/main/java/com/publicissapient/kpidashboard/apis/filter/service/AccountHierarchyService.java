...
public interface AccountHierarchyService<R, S> {
	...
	default List<ProjectHierarchy> getConfigureProjectsHierarchies(
		...
		List<ObjectId> projectBasicConfigIds =
				projectBasicConfigList.stream().map(ProjectBasicConfig::getId).collect(Collectors.toList());

		// required basicConfigId in final response list on project level but
		// OrganizationHierarchy do not have basicConfigId.
		Map<String, ProjectBasicConfig> projectNodeWiseBasicConfigIdMap =
				projectBasicConfigList.stream()
						.collect(
								Collectors.toMap(
										ProjectBasicConfig::getProjectNodeId, Function.identity(), (e1, e2) -> e1));

		// required only configured Project Nodes and Above their Hierarchy Nodes
		// filters from OrganizationHierarchy Collections
		Set<String> hierarchyNodes =
				projectBasicConfigList.stream()
						.flatMap(a -> a.getHierarchy().stream())
						.map(hierarchyValue -> hierarchyValue.getOrgHierarchyNodeId())
						.collect(Collectors.toSet());

		hierarchyNodes.addAll(
				projectBasicConfigList.stream()
						.map(ProjectBasicConfig::getProjectNodeId)
						.collect(Collectors.toSet()));

		List<OrganizationHierarchy> configureOrganizationHierarchyList =
				organizationHierarchyService.findAll().stream()
						.filter(
								organizationHierarchy -> hierarchyNodes.contains(organizationHierarchy.getNodeId()))
						.toList();

		// required only configured Project Below filters like sprint , squad , releases
		List<ProjectHierarchy> configureHierarchies =
				projectHierarchyService.findAllByBasicProjectConfigIds(projectBasicConfigIds);
						...
						orgHierarchy -> {
							if (orgHierarchy.getHierarchyLevelId().equalsIgnoreCase(CommonConstant.PROJECT)) {
								ProjectBasicConfig projectBasicConfig =
										projectNodeWiseBasicConfigIdMap.get(orgHierarchy.getNodeId());
								return new ProjectHierarchy(
										orgHierarchy.getNodeId(),
										orgHierarchy.getNodeName(),
										orgHierarchy.getNodeDisplayName(),
										orgHierarchy.getHierarchyLevelId(),
										orgHierarchy.getParentId(),
										orgHierarchy.getCreatedDate(),
										orgHierarchy.getModifiedDate(),
										projectBasicConfig.getId(),
										orgHierarchy.getCreatedBy(),
										orgHierarchy.getUpdatedBy(),
										orgHierarchy.getExternalId(),
										projectBasicConfig.isProjectOnHold());
							} else {
								return new ProjectHierarchy(
										orgHierarchy.getNodeId(),
										orgHierarchy.getNodeName(),
