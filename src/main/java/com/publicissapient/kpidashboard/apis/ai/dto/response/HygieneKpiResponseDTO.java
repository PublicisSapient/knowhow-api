 ...
 *
 * <p>The LLM produces a JSON ARRAY — one element per input issue. Consumers should
 * deserialize into {@code List<HygieneKpiResponseDTO>}; each element of that list
 * carries the per-rule verdict, aggregated counts / score / grade, overall
 * readiness, top failures, and pipe-separated improvement recommendations.
 *
 * <p>String-typed enum-like fields are intentionally kept as {@link String} to be
 * forgiving of minor LLM formatting drift. The expected vocabulary is:
 * <ul>
 *   <li>{@code results[].status} — {@code "Passed" | "Failed" | "Partial" | "N/A"}</li>
 *   <li>{@code hygieneGrade}     — {@code "GOOD" | "AVERAGE" | "POOR"}</li>
 *   <li>{@code overallStatus}    — {@code "READY" | "NOT READY"}</li>
 * </ul>
 */
@Data
@Builder
...
public class HygieneKpiResponseDTO {
    ...
    /**
     * Hygiene score in the range {@code [0, 100]}, computed as
     * {@code passedRules * 100 / totalApplicableRules}
     * (falls back to {@code 100} when there are no applicable rules).
     */
    ...
    /**
     * {@code "READY"} only when every applicable rule is {@code "Passed"};
     * otherwise {@code "NOT READY"}.
     */
    ...
    /**
     * 3–5 short, actionable improvement suggestions joined by {@code " | "}.
     * Stored as a single {@link String} to match the CSV/Excel-friendly contract
     * declared in the hygiene analyzer prompt.
     */
