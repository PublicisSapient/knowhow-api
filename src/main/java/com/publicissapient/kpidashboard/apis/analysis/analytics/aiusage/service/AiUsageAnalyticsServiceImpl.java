...
public class AiUsageAnalyticsServiceImpl implements AiUsageAnalyticsService {
	...
	private static final String COMPUTING_AI_ANALYTICS_FAILED_ERROR_MESSAGE =
			"Computing the AI usage analytics failed";
	...
	public ServiceResponse computeAiUsageAnalyticsData(
			@Valid BaseAnalyticsRequestDTO aiUsageAnalyticsRequestDTO) {
		...
		validateBaseAnalyticsComputationRequest(
				aiUsageAnalyticsRequestDTO, projectsDataCurrentUserHasAccessTo);

		ServiceResponse serviceResponse = new ServiceResponse();
		serviceResponse.setData(
				computeAiUsageAnalytics(aiUsageAnalyticsRequestDTO, projectsDataCurrentUserHasAccessTo));

		return serviceResponse;
	}

	private AiUsageAnalyticsResponseDTO computeAiUsageAnalytics(
			BaseAnalyticsRequestDTO aiUsageAnalyticsRequestDTO,
			List<AccountFilteredData> projectsDataCurrentUserHasAccessTo) {
		List<BaseProjectForAnalyticsDTO> projectDTOList =
				extractProjectDataForAnalyticsComputation(
						projectsDataCurrentUserHasAccessTo,
						aiUsageAnalyticsRequestDTO.getProjectBasicConfigIds());

		if (CollectionUtils.isEmpty(projectDTOList)) {
			log.error(
					"Could not create the project AI usage analytics computation data list for request {}",
		...
		Map<String, List<JiraIssue>> basicProjectConfigIdAsStringJiraIssuesMap =
				createProjectBasicConfigIdAndAssociatedJiraIssuesMap(
						projectDTOList, aiUsageAnalyticsRequestDTO);

		if (MapUtils.isEmpty(basicProjectConfigIdAsStringJiraIssuesMap)) {
			log.warn(
					"No Jira Issues could be found for project basic config ids present in the AI usage analytics "
							+ "request {}",
					aiUsageAnalyticsRequestDTO);
			return AiUsageAnalyticsResponseDTO.builder().build();
		}

		return constructAiUsageAnalyticsResponseDTO(
	...
	private Map<String, List<JiraIssue>> createProjectBasicConfigIdAndAssociatedJiraIssuesMap(
			List<BaseProjectForAnalyticsDTO> projectDTOList,
			BaseAnalyticsRequestDTO aiUsageAnalyticsRequestDTO) {
		List<SprintDetails> sprintsTakenIntoConsideration =
				sprintDetailsService.findByBasicProjectConfigIdInByCompletedDateDesc(
						projectDTOList.stream()
								.map(BaseProjectForAnalyticsDTO::getProjectBasicConfigId)
								.toList(),
						aiUsageAnalyticsRequestDTO.getNumberOfSprintsToInclude());

		Map<ObjectId, List<SprintDetails>> basicProjectConfigIdSprintDetailsMap =
				sprintsTakenIntoConsideration.stream()
		...
		for (BaseProjectForAnalyticsDTO projectDTO : projectDTOList) {
			jiraIssueNumbers.addAll(
					getJiraIssueNumbersForProject(
		...
		List<JiraIssue> jiraIssues =
				jiraIssueRepository.findByNumberInAndBasicProjectConfigIdIn(
						jiraIssueNumbers, aiUsageAnalyticsRequestDTO.getProjectBasicConfigIds());

		return jiraIssues.stream().collect(Collectors.groupingBy(JiraIssue::getBasicProjectConfigId));
	}

	private Set<String> getJiraIssueNumbersForProject(
			ObjectId projectBasicConfigId, List<SprintDetails> projectSprintDetails) {
		if (CollectionUtils.isEmpty(projectSprintDetails)) {
			return Collections.emptySet();
		}
		Set<String> projectJiraIssueNumbers;
		FieldMapping projectFieldMapping =
				configHelperService.getFieldMappingMap().get(projectBasicConfigId);
		List<String> includedStatusesToIdentifyCompletedIssuesKPI198 =
				projectFieldMapping.getIncludedStatusesToIdentifyCompletedIssuesKPI198();
		...
		if (CollectionUtils.isNotEmpty(includedStatusesToIdentifyCompletedIssuesKPI198)) {
			projectJiraIssueNumbers =
					projectSprintDetails.stream()
							.flatMap(sprintDetails -> sprintDetails.getTotalIssues().stream())
							.filter(
									sprintIssue -> {
										if (CollectionUtils.isNotEmpty(
												includedIssueTypesFromTheCompletedIssuesKPI198)) {
											return includedStatusesToIdentifyCompletedIssuesKPI198.contains(
															sprintIssue.getStatus())
													&& includedIssueTypesFromTheCompletedIssuesKPI198.contains(
															sprintIssue.getTypeName());
										}
										return includedStatusesToIdentifyCompletedIssuesKPI198.contains(
												sprintIssue.getStatus());
									})
			...
			projectJiraIssueNumbers =
					projectSprintDetails.stream()
							.flatMap(sprintDetails -> sprintDetails.getCompletedIssues().stream())
							.filter(
									sprintIssue -> {
										if (CollectionUtils.isNotEmpty(
												includedIssueTypesFromTheCompletedIssuesKPI198)) {
											return includedIssueTypesFromTheCompletedIssuesKPI198.contains(
													sprintIssue.getTypeName());
										}
										return true;
									})
	...
	private static Map<String, Map<String, ProjectAiUsageMetrics>>
			groupProjectsByAiUsageTypeAndBasicProjectConfigId(
					List<BaseProjectForAnalyticsDTO> projectDTOList,
					Map<String, List<JiraIssue>> basicProjectConfigIdAsStringJiraIssuesMap) {
		Map<String, Map<String, ProjectAiUsageMetrics>>
				aiUsageTypeBasicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap = new HashMap<>();
		for (BaseProjectForAnalyticsDTO projectDTO : projectDTOList) {
			if (basicProjectConfigIdAsStringJiraIssuesMap.containsKey(
					projectDTO.getProjectBasicConfigId().toString())) {
				basicProjectConfigIdAsStringJiraIssuesMap
						.get(projectDTO.getProjectBasicConfigId().toString())
						.forEach(
								jiraIssue -> {
									if (StringUtils.isNotEmpty(jiraIssue.getAiUsageType())) {
										...
										Map<String, ProjectAiUsageMetrics>
												basicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap =
														aiUsageTypeBasicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap
																.get(jiraIssue.getAiUsageType());
										String projectBasicConfigIdAsString =
												projectDTO.getProjectBasicConfigId().toString();
										if (basicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.containsKey(
												projectBasicConfigIdAsString)) {
											ProjectAiUsageMetrics projectAiUsageMetrics =
													basicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.get(
															projectBasicConfigIdAsString);
											projectAiUsageMetrics.setIssueCount(
													projectAiUsageMetrics.getIssueCount() + 1);
											projectAiUsageMetrics.setEfficiencyGain(
													(double)
																	Math.round(
																			(projectAiUsageMetrics.getEfficiencyGain()
																							+ (jiraIssue.getAiEfficiencyGain()
				...
				log.warn(
						"No AI usage analytics data could be found for project with basicProjectConfigID {}",
						projectDTO.getProjectBasicConfigId());
			}
		}
	...
	private static AiUsageAnalyticsResponseDTO constructAiUsageAnalyticsResponseDTO(
		...
		int aiUsageTypesNumber =
				aiUsageTypeBasicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.size();
		...
		double totalEfficiencyGain =
				aiUsageTypeBasicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.values().stream()
						.flatMap(
								basicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap ->
										basicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.values().stream())
						.mapToDouble(ProjectAiUsageMetrics::getEfficiencyGain)
						.sum();

		aiUsageAnalyticsSummaryDTO.setAverageEfficiencyGainPerAiUsageType(
				(int) Math.round(totalEfficiencyGain / aiUsageTypesNumber));
		aiUsageAnalyticsSummaryDTO.setAverageEfficiencyGainPerProject(
				(int) Math.round(totalEfficiencyGain / projectsNumber));

		List<AiUsageAnalyticsDTO> aiUsageAnalyticsDTOList = new ArrayList<>();

		for (Map.Entry<String, Map<String, ProjectAiUsageMetrics>> entry :
				aiUsageTypeBasicProjectConfigIdAsStringProjectAiUsageAnalyticsDTOMap.entrySet()) {
		...
		return AiUsageAnalyticsResponseDTO.builder()
				.summary(aiUsageAnalyticsSummaryDTO)
				.analytics(aiUsageAnalyticsDTOList)
				.build();
	}
		...
		if (CollectionUtils.isEmpty(requestedBasicProjectConfigIds)) {
			projectDTOList =
					projectsDataCurrentUserHasAccessTo.stream()
							.map(
									accountFilteredData ->
											new BaseProjectForAnalyticsDTO(
													accountFilteredData.getNodeDisplayName(),
			...
			projectDTOList =
					projectsDataCurrentUserHasAccessTo.stream()
							.filter(
									accountFilteredData ->
											requestedBasicProjectConfigIds.contains(
