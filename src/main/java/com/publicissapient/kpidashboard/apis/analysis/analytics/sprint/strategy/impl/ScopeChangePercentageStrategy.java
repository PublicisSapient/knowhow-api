 ...
 *
 * <p>Identifies stories with scope modifications during the sprint using configured scope change
 * labels.
 *
 * <h3>Calculation Logic:</h3>
 *
 * <ul>
 *   <li>Identify stories with scope change labels during sprint
 *   <li>Uses configured scope change labels from field mapping (jiraLabelsKPI120)
 * </ul>
 *
 * <h3>Formula:</h3>
 *
...
public class ScopeChangePercentageStrategy extends AbstractSprintMetricStrategy {
	...
	protected SprintDataPoint calculateForSprint(
		...
		if (CollectionUtils.isEmpty(scopeChangeLabels)) {
			...
			return createNADataPoint(
					sprintDetails,
					"Scope change labels not configured in field mapping",
					sprintIndex,
					context);
		}
	 ...
	 * @param scopeChangeLabels List of scope change label values
	 * @return Count of issues with scope change labels
	 */
	private int countScopeChangeIssues(
			SprintDetails sprintDetails, SprintMetricContext context, List<String> scopeChangeLabels) {

		// Get all sprint issue numbers (total + punted)
		List<String> allIssueNumbers =
		...
		List<JiraIssue> scopeChangeIssues =
				allIssueNumbers.stream()
						.map(issueNumber -> context.getJiraIssueByNumber(issueNumber).orElse(null))
						.filter(Objects::nonNull)
						.filter(issue -> hasAnyScopeChangeLabel(issue, scopeChangeLabels))
						.distinct()
						.toList();
	 ...
	 * @param scopeChangeLabels Configured scope change labels
