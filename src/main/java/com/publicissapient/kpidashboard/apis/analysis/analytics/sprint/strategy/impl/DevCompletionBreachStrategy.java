 ...
 *
 * <p>Tracks stories that failed to reach "Dev Done" status by their configured Dev Completion Date.
 *
 * <h3>Calculation Logic:</h3>
 *
 * <ul>
 *   <li>Track stories not dev-completed by their Dev Completion Date
 *   <li>Uses devDueDates and dev done statuses from field mapping
 *   <li>Checks statusUpdationLog from JiraIssueCustomHistory for status transitions
 * </ul>
 *
 * <h3>Formula:</h3>
 *
 ...
 * <blockquote>
 *
 * <b>10 stories</b> have Dev Completion Date set<br>
 * <b>3 stories</b> did NOT reach "Dev Done" status by their due date<br>
 * <b>Value</b> = 3<br>
 * <b>Trend</b> = (3/10) × 100 = <b>30%</b>
 *
 * </blockquote>
 *
...
public class DevCompletionBreachStrategy extends AbstractSprintMetricStrategy {
	...
	protected SprintDataPoint calculateForSprint(
		...
		int breachCount =
				countDevCompletionBreach(issuesWithDevDueDate, sprintDetails, context, devDoneStatuses);
	 ...
	 */
	private List<JiraIssue> getIssuesWithDevDueDate(
			SprintDetails sprintDetails, SprintMetricContext context) {
		return sprintDetails.getTotalIssues().stream()
				.map(SprintIssue::getNumber)
				.map(issueNumber -> context.getJiraIssueByNumber(issueNumber).orElse(null))
				.filter(Objects::nonNull)
				.filter(issue -> StringUtils.isNotEmpty(issue.getDevDueDate()))
				.collect(Collectors.toList());
	}

	/**
	 * Counts issues that breached dev completion date Logic Active Sprint: devDueDate < today AND NOT
	 * dev-completed - Closed Sprint: devDueDate <= sprintEndDate AND NOT dev-completed
	 *
	 * @param issuesWithDevDueDate Issues with dev due date
	 * @param sprintDetails Sprint details
	 * @param context Metric context
	 * @param devDoneStatuses Dev done statuses from field mapping
	 * @return Count of breached issues
	 */
	private int countDevCompletionBreach(
			List<JiraIssue> issuesWithDevDueDate,
		...
		return (int)
				issuesWithDevDueDate.stream()
						.filter(
								issue -> {
									LocalDate devDueDate =
											DateUtil.stringToLocalDate(
													issue.getDevDueDate(), DateUtil.TIME_FORMAT_WITH_SEC);
									...
									boolean isDevCompleted =
											isDevCompleted(issue, context, devDoneStatuses, sprintDetails);
	...
	/**
	 * Checks if issue is dev-completed Uses Find when issue reached dev-done status from
	 * statusUpdationLog
	 *
	 * @param issue Jira issue
	 * @param context Metric context
	 * @param devDoneStatuses Dev done statuses
	 * @param sprintDetails Sprint details for date window
	 * @return true if issue reached dev-done status within sprint window
	 */
	private boolean isDevCompleted(
			JiraIssue issue,
			SprintMetricContext context,
			List<String> devDoneStatuses,
			SprintDetails sprintDetails) {
