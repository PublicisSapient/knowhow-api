...
public interface AIUsageAPI {
	...
	@Operation(
			...
			responses = {
				@ApiResponse(
						...
						content =
								@Content(
										schema = @Schema(implementation = InitiateUploadResponse.class),
										examples =
												@ExampleObject(
				...
				@ApiResponse(
						...
						content =
								@Content(
										schema =
												@Schema(
														implementation = HttpServerErrorException.InternalServerError.class)))
			})
	@PostMapping("/upload/path")
	InitiateUploadResponse uploadAiUsageData(
			@Parameter(
							description = "Absolute path to the CSV file to be processed",
							required = true,
							example = "/absolute/path/to/your/file.csv")
					@RequestBody
					UploadAIUsageRequest filePath);
	...
	InitiateUploadResponse uploadAiUsageDataFile(
			@Parameter(description = "CSV file to be processed", required = true) @RequestPart("file")
					MultipartFile file);

	@Operation(
			summary = "Get processing status",
			description =
					"Retrieves the current processing status of a CSV upload request using the request ID.",
			responses = {
				@ApiResponse(
						responseCode = "200",
