...
public class AIUsageAsyncProcessingAspect {
	...
	@AfterReturning(
			pointcut =
					"execution(* com.publicissapient.kpidashboard.apis.aiusage.service.AIUsageService.uploadFile(..))",
			returning = "response")
	@Transactional
		...
		CompletableFuture.runAsync(
				() -> {
					try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
						...
						while ((line = reader.readLine()) != null) {
							...
							AIUsage aiUsage =
									createAIUsageFromColumns(
											columns, columnIndexMap, headerMapping, failedRecordsCount, uploadStatus);
						...
						if (uploadStatus.getStatus() != UploadStatus.FAILED
								&& Objects.equals(failedRecordsCount.get(), 0)) {
						...
						log.info(
								"Final status for requestId {}: Status={}, TotalRecords={}, SuccessfulRecords={}, FailedRecords={}",
								requestId,
								uploadStatus.getStatus(),
								uploadStatus.getTotalRecords(),
								uploadStatus.getSuccessfulRecords(),
								uploadStatus.getFailedRecords());
						aiUsageService.saveUploadStatus(uploadStatus);
					}
				});
	}

	private AIUsage createAIUsageFromColumns(
			String[] columns,
			Map<String, Integer> columnIndexMap,
			Map<String, String> headerMapping,
			AtomicInteger failedRecordsCount,
	...
	private static void setValuesInDocument(
			AtomicInteger failedRecordsCount,
			AIUsageRequest uploadStatus,
			String mappedField,
			AIUsage aiUsage,
