 ...
 * @param <S> Trend object type
 * @param <T> Bind DB data with type
 * @author prigupta8
 */
public abstract class SonarKPIService<R, S, T> extends ToolsKPIService<R, S>
	 ...
	 * @return
	 */
	private List<SonarHistory> getSonarHistoryBasedOnProject(
			ObjectId projectId, LocalDate currentDate) {
		...
		if (null != configHelperService.getToolItemMap()
			...
			List<Tool> sonarConfigListBasedOnProject =
					configHelperService.getToolItemMap().get(projectId).get(Constant.TOOL_SONAR);
			...
			if (CollectionUtils.isNotEmpty(sonarConfigListBasedOnProject)) {
				sonarConfigListBasedOnProject.forEach(
						job -> {
							if (CollectionUtils.isNotEmpty(job.getProcessorItemList())) {
								...
								List<SonarHistory> sonarHistoryList =
										sonarHistoryRepository.findByProcessorItemIdInAndTimestampGreaterThan(
												processorItemList, timestamp);
	 ...
	 * @return
	 */
	public Map<Pair<String, String>, List<SonarHistory>> getSonarHistoryForAllProjects(
			List<Node> projectList, LocalDate currentDate) {
		Map<Pair<String, String>, List<SonarHistory>> map = new HashMap<>();
		projectList.stream()
				.filter(
						node ->
								null != node.getProjectFilter()
										&& null != node.getProjectFilter().getBasicProjectConfigId())
				.forEach(
						node ->
								map.put(
										Pair.of(node.getId(), node.getProjectFilter().getName()),
										getSonarHistoryBasedOnProject(
												node.getProjectFilter().getBasicProjectConfigId(), currentDate)));
		return map;
	}
	 ...
	 * @param value value of how many days/week/months
	 * @return
	 */
	 ...
	 * @param sonarKeyName sonarKeyName
	 * @param branchName sonar branch name
	...
	public String prepareSonarKeyName(String projectName, String sonarKeyName, String branchName) {
		...
		if (StringUtils.isNotEmpty(branchName)) {
			sonarKey =
					sonarKeyName + CommonConstant.ARROW + branchName + CommonConstant.ARROW + projectName;
		}
		return sonarKey;
	}
	...
	public Map<String, SonarHistory> prepareJobwiseHistoryMap(
		...
		for (SonarHistory sonarHistory : sonarHistoryList) {
			if (sonarHistory.getTimestamp().compareTo(start) > 0
				...
				if (sonarHistory.getTimestamp().compareTo(map.get(sonarHistory.getKey()).getTimestamp())
						> 0) {
	...
	public DataCount getDataCountObject(
			Long value, Map<String, Object> hoverValues, String projectName, String date) {
	 ...
	 *
	 * @param leafNodeList A list of leaf nodes for which KPI data needs to be fetched.
	 * @return A map where the key is the project ID and the value is a list of objects
	 *     (SprintDetails) associated with that project.
	 */
	protected Map<String, SprintDetails> getSprintDetailsByIds(List<Node> leafNodeList) {
		...
		if (CollectionUtils.isNotEmpty(leafNodeList)) {
			...
			List<SprintDetails> sprintDetailsList =
					sprintDetailsService.getSprintDetailsByIds(
							processedProjectIds.values().stream().toList());
			processedProjectIds.forEach(
					(key, value) -> {
						Optional<SprintDetails> filtered =
