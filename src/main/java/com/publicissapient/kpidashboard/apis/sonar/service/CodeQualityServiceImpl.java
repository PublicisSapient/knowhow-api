...
/**
 * this kpi is used to find out the code quality of any branch you configure on sonar
 *
 * @author shi6
 */
@Component
...
public class CodeQualityServiceImpl
	...
	public KpiElement getKpiData(
		...
		calculateAggregatedValueMap(
				treeAggregatorDetail.getRoot(), nodeWiseKPIValue, KPICode.SONAR_CODE_QUALITY);

		Map<String, List<DataCount>> trendValuesMap =
				getTrendValuesMap(kpiRequest, kpiElement, nodeWiseKPIValue, KPICode.SONAR_CODE_QUALITY);
		...
		log.debug(
				"[SONAR-TECH-DEBT-AGGREGATED-VALUE][{}]. Aggregated Value at each level in the tree {}",
				kpiRequest.getRequestTrackerId(),
				treeAggregatorDetail.getRoot());
		return kpiElement;
	}
	...
	public void getSonarKpiData(
		...
		getSonarHistoryForAllProjects(
						...
						getScrumCurrentDateToFetchFromDb(
								CommonConstant.MONTH, Long.valueOf(customApiConfig.getSonarMonthCount())))
				.forEach(
							...
							if (CollectionUtils.isNotEmpty(projectData)) {
								...
								SprintDetails sprintDetails =
										sprintDetailsList.get(projectId) != null
												...
												: null;
								processProjectData(projectNodePair, projectData, sprintDetails, tempMap, excelData);
							}
						});
	...
	private void processProjectData(
		...
		if (sprintDetails != null) {
			endDateTime =
					sprintDetails.getCompleteDate() != null
							? DateUtil.stringToLocalDate(
									sprintDetails.getCompleteDate().replaceAll(DATE_TIME_FORMAT_REGEX, ""),
									DateUtil.TIME_FORMAT)
							: DateUtil.stringToLocalDate(
									sprintDetails.getEndDate().replaceAll(DATE_TIME_FORMAT_REGEX, ""),
									DateUtil.TIME_FORMAT);
		}

		for (int i = 0; i < customApiConfig.getSonarMonthCount(); i++) {
			CustomDateRange dateRange =
					KpiDataHelper.getStartAndEndDateForDataFiltering(endDateTime, CommonConstant.MONTH);
			...
			String date =
					DateUtil.dateTimeConverter(
									monthStartDate.toString(), DateUtil.DATE_FORMAT, DateUtil.DISPLAY_DATE_FORMAT)
							+ " to "
							+ DateUtil.dateTimeConverter(
									monthEndDate.toString(), DateUtil.DATE_FORMAT, DateUtil.DISPLAY_DATE_FORMAT);
			...
			prepareSqualeList(
					history, date, projectNodePair, projectList, debtList, projectWiseDataMap, versionDate);
		...
		if (getRequestTrackerId().toLowerCase().contains(KPISource.EXCEL.name().toLowerCase())) {
			KPIExcelUtility.populateSonarKpisExcelData(
					...
					versionDate,
					excelData,
					KPICode.SONAR_CODE_QUALITY.getKpiId());
		}
	}
	 ...
	 * @return map
	 */
	public Map<String, SonarHistory> prepareEmptyJobWiseHistoryMap(
			List<SonarHistory> sonarHistoryList, Long end) {
		...
		if (CollectionUtils.isNotEmpty(sonarHistoryList)) {
			...
			List<String> uniqueKeys =
					sonarHistoryList.stream().map(SonarHistory::getKey).distinct().toList();
			uniqueKeys.forEach(
					keys -> {
						SonarHistory sonarHistory =
								SonarHistory.builder()
										.processorItemId(refHistory.getProcessorItemId())
										.date(end)
										.timestamp(end)
										.key(keys)
										.name(keys)
										.branch(refHistory.getBranch())
										.metrics(metricsList)
										.build();
	 ...
	 *
	 * @param history sonarhistory
	 * @param date node date
	 * @param projectNodePair projectNodePair
	 * @param projectList projectList
	 * @param debtList db debtList
	 * @param projectWiseDataMap projectWiseDataMap
	 * @param versionDate versionDate
	 */
	private void prepareSqualeList(
		...
		history
				...
				.forEach(
						sonarDetails -> {
							...
							String keyName =
									prepareSonarKeyName(
											projectNodePair.getValue(), sonarDetails.getName(), sonarDetails.getBranch());
		...
		DataCount dcObj =
				getDataCount(
						calculateKpiValue(dateWiseDebtList, KPICode.SONAR_CODE_QUALITY.getKpiId()),
	...
	/**
	 * Get all the code quality debt data in form of key as sonar-project and value as techDebt
	 * sqale_index value
	 *
	 * @param pList : project list of nodes
	 * @param tempMap : containing all nodes of the hierarchy with key as node id and value as node
	 * @param kpiElement : request info
	 * @return map having key as sonar-project and value as techDebt sqale_index value
	 */
	public Map<String, Object> getSonarJobWiseKpiData(
			final List<Node> pList, Map<String, Node> tempMap, final KpiElement kpiElement) {
		return new HashMap<>();
	}
	...
	public Double calculateThresholdValue(FieldMapping fieldMapping) {
		return calculateThresholdValue(
				fieldMapping.getThresholdValueKPI168(), KPICode.SONAR_CODE_QUALITY.getKpiId());
	}
}
