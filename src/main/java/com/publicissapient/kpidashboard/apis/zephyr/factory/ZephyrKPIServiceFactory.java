...
/**
 * This class loads all Jira KPIs services and map into Map on the startup of server.
 *
 * @author tauakram
 */
@Service
...
public class ZephyrKPIServiceFactory {
	...
	private static final Map<String, ZephyrKPIService<?, ?, ?>> ZEPHYR_SERVICE_CACHE =
			new HashMap<>();
	@Autowired private List<ZephyrKPIService<?, ?, ?>> services;
	...
	public static ZephyrKPIService getZephyrKPIService(String type) throws ApplicationException {
		...
		if (service == null) {
			throw new ApplicationException(
					ZephyrKPIServiceFactory.class, "Zephyr KPI Service Factory not initalized");
		}
		return service;
	}

	/**
	 * This method put all available Jira services to Map where key is the KPI id and value is the
	 * service object.
	 */
	@PostConstruct
