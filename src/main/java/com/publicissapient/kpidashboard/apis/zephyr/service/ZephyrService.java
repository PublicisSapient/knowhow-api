...
/**
 * Service to calculate the Zephyr KPI. This service interacts with the cache (for list of builds),
 * mongoDB (for KPI data build wise) and aggregationService (for aggregation).
 *
 * @author tauakram
...
public class ZephyrService {
	...
	public List<KpiElement> process(KpiRequest kpiRequest) throws EntityNotFoundException {
		...
		log.info(
				"[ZEPHYR][{}]. Processing KPI calculation for data {}",
				kpiRequest.getIds(),
		...
		try {
			...
			if (!CollectionUtils.isEmpty(filteredAccountDataList)) {
				...
				for (KpiElement kpiEle : kpiRequest.getKpiList()) {
					...
					listOfTask.add(
							new ParallelZephyrServices(kpiRequest, responseList, kpiEle, treeAggregatorDetail));
				}
			...
			log.error(
					"[ZEPHYR][{}]. Error while KPI calculation for data {} {}",
					kpiRequest.getIds(),
					kpiRequest.getKpiList(),
					e);
		...
		private KpiElement calculateAllKPIAggregatedMetrics(
			...
			try {
				...
				if (!projectNodes.isEmpty()
					...
					kpiElement =
							zephyrKPIService.getKpiData(kpiRequest, kpiElement, treeAggregatorDetailClone);
	 ...
	 *
	 * @param kpiRequest Zephyr KPI request true if flow for precalculated, false for direct flow.
	 * @param withCache
