...
public class AiKpiRecommendationProviderServiceImpl implements KpiRecommendationProviderService {
	...
	@Autowired KpiIntegrationServiceImpl kpiIntegrationService;
	 ...
	 *     each project.
	 * @throws AiGatewayServiceException if an error occurs while retrieving AI recommendations.
	 */
	@Override
	public List<ProjectWiseKpiRecommendation> getProjectWiseKpiRecommendations(
		...
		try {
			...
			ChatGenerationRequest chatGenerationRequest =
					ChatGenerationRequest.builder().prompt(prompt).build();
	 ...
	 *
	 * @param kpiRequest The request object containing KPI elements and their associated data.
	 * @return A map where the key is the KPI name and the value is a list of formatted KPI data
	 *     prompts.
	 */
	...
	/**
	 * Builds a list of project-wise KPI recommendations based on the provided KPI request and
	 * response object.
	 *
	 * @param kpiRequest The request object containing project and KPI details.
	 * @param responseObject The response object containing project health and recommendations data.
	 * @return A list containing a single {@link ProjectWiseKpiRecommendation} object with project
	 *     details and recommendations.
	 */
	private List<ProjectWiseKpiRecommendation> buildProjectWiseRecommendations(
			KpiRequest kpiRequest, Object responseObject) {
		...
		recommendation.setProjectScore(
				((ObjectNode) responseObject).get("project_health_value").asDouble());
		...
		jsonArray.forEach(
				jsonElement -> {
					...
					genericRecommendationItem.setObservation(
							String.valueOf(jsonElement.get("observation").asText()));
					genericRecommendationItem.setRecommendationDetails(
							String.valueOf(jsonElement.get("recommendation").asText()));
					genericRecommendationItem.setKpiName(String.valueOf(jsonElement.get("kpi").asText()));
					genericRecommendationItem.setObservation(
							String.valueOf(jsonElement.get("observation").asText()));
					genericRecommendationItem.setRecommendationType(
							String.valueOf(jsonElement.get("severity").asText()));
