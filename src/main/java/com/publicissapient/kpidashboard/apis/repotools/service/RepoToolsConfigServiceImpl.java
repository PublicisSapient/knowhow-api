...
public class RepoToolsConfigServiceImpl {
	...
	@Autowired private RepoToolsClient repoToolsClient;
	 ...
	 */
	public ServiceResponse configureRepoToolProject(
			ProjectToolConfig projectToolConfig, Connection connection, List<String> branchNames) {
		try {
	 ...
	 */
	private void setToolWiseRepoToolConfig(
			Connection connection, ProjectToolConfig projectToolConfig, RepoToolConfig repoToolConfig) {
		// Split the Git URL to extract the repository name
		String[] split = projectToolConfig.getGitFullUrl().split("/");
		String name = split[split.length - 1];
		if (name.contains(".")) name = name.split(".git")[0];
		...
		switch (projectToolConfig.getToolName()) {
			...
			case Constant.TOOL_BITBUCKET:
				...
				repoToolConfig.setApiEndPoint(
						connection.getApiEndPoint() + PROJECT + split[split.length - 2] + REPOS + name);
		...
		repoToolConfig.setScanningAccount(
				new ToolCredential(
	...
	public int triggerScanRepoToolProject(String processorName, String basicProjectConfigId) {
		...
		List<ProjectToolConfig> projectRepos =
				projectToolConfigRepository.findByToolNameAndBasicProjectConfigId(
						processorName, new ObjectId(basicProjectConfigId));
		try {
			...
			if (CollectionUtils.isNotEmpty(projectToolConfigList)) {
				...
				httpStatus =
						repoToolsClient.triggerScanCall(
								projectCode,
	 ...
	 *
	 * @param toolList the list of ProjectToolConfig objects representing the tools associated with
	 *     the project
	 ...
	 */
	public boolean updateRepoToolProjectConfiguration(
			List<ProjectToolConfig> toolList, ProjectToolConfig tool, String basicProjectConfigId) {
		int httpStatus = HttpStatus.NOT_FOUND.value();
		List<ProjectToolConfig> mutableToolList = new ArrayList<>(toolList);
		try {
			if (mutableToolList.size() > 1) {
				Optional<ProjectToolConfig> projectToolConfig =
						mutableToolList.stream()
								.filter(toolConfig -> toolConfig.getId().equals(tool.getId()))
								.findFirst();
				...
				if (CollectionUtils.isNotEmpty(getToolByRepo(tool, mutableToolList))) {
					...
					String deleteRepoUrl =
							customApiConfig.getRepoToolURL()
									+ String.format(
											customApiConfig.getRepoToolDeleteRepoUrl(),
											URLEncoder.encode(
													createProjectCode(basicProjectConfigId), StandardCharsets.UTF_8),
											tool.getRepositoryName());
					httpStatus =
							repoToolsClient.deleteRepositories(
									deleteRepoUrl, customApiConfig.getRepoToolAPIKey());
				}
			} else {
	 ...
	 */
	private List<ProjectToolConfig> getToolByRepo(
			ProjectToolConfig tool, List<ProjectToolConfig> projectToolConfigList) {
		if (tool.getToolName().equalsIgnoreCase(Constant.TOOL_GITLAB))
			return projectToolConfigList.stream()
					.filter(toolConfig -> toolConfig.getProjectId().equals(tool.getProjectId()))
					.toList();
		else if (tool.getToolName().equalsIgnoreCase(Constant.TOOL_BITBUCKET))
			return projectToolConfigList.stream()
					.filter(toolConfig -> toolConfig.getRepoSlug().equals(tool.getRepoSlug()))
					.toList();
		else {
			return projectToolConfigList.stream()
					.filter(toolConfig -> toolConfig.getRepositoryName().equals(tool.getRepositoryName()))
					.toList();
		}
	}
	 ...
	 * @return
	 */
	public List<RepoToolKpiMetricResponse> getRepoToolKpiMetrics(
		...
		RepoToolKpiRequestBody repoToolKpiRequestBody =
				new RepoToolKpiRequestBody(
						projectCode.stream().map(code -> code.replaceAll("\\s", "")).toList(),
						startDate,
						endDate,
						frequency);
		try {
			...
			repoToolKpiMetricRespons =
					repoToolKpiBulkMetricResponse.getValues().stream().flatMap(List::stream).toList();
		} catch (Exception ex) {
	...
	/**
	 * Creates a list of ProcessorItem objects from a list of ProjectToolConfig objects and a
	 * Processor.
	 *
	 * @param toolList the list of ProjectToolConfig objects
	 * @param processor the Processor object
	 * @return a list of ProcessorItem objects
	 */
	private List<ProcessorItem> createProcessorItemList(
			List<ProjectToolConfig> toolList, Processor processor) {
	...
	public void saveRepoToolProjectTraceLog(RepoToolsStatusResponse repoToolsStatusResponse) {
		...
		Optional<ProcessorExecutionTraceLog> existingTraceLogOptional =
				processorExecutionTraceLogRepository.findByProcessorNameAndBasicProjectConfigId(
						repoToolsStatusResponse.getRepositoryProvider(), basicProjectConfigId);
		...
		if (Boolean.TRUE.equals(isWarning))
			processorExecutionTraceLog.setExecutionResumesAt(
					LocalDateTime.now().plusHours(1).toInstant(ZoneOffset.UTC).toEpochMilli());
	...
	public int updateRepoToolConnection(Connection conn) {
		...
		try {
			RepoToolsProvider repoToolsProvider =
					repoToolsProviderRepository.findByToolName(conn.getRepoToolProvider().toLowerCase());
			...
			repoToolConnectionDetail.setPassword(
					aesEncryptionService.decrypt(
							conn.getAccessToken(), customApiConfig.getAesEncryptionKey()));
			...
			repoToolsClient.updateConnection(
					repoToolConnModel,
	...
	public JsonNode getProjectRepoToolMembers(String basicProjectConfigId) {
		...
		try {
			...
			jsonNode =
					repoToolsClient.fetchProjectRepoToolMembers(
							membersUrl, customApiConfig.getRepoToolAPIKey());
		} catch (Exception ex) {
