...
public class AccessRequestsController {
	...
	@Operation(
			...
			description = "Fetches all access requests data. Pre-authorization is required for this action."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	@Operation(
			...
			description = "Fetches access request data for a specific ID. Pre-authorization is required for this action."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	@Operation(
			...
			description = "Fetches all access requests data associated with the specified username. " +
	...
	public ResponseEntity<ServiceResponse> getAccessRequestByUsername(
			@Parameter(description = "The username whose access requests are to be retrieved", example = "john_doe")
	...
	@Operation(
			...
			description = "Fetches all access requests data that match the specified status. " +
					"Pre-authorization is required for this action."
	)
	@ApiResponses(value = {
			@ApiResponse(
	...
	@Operation(
			...
			description = "Updates the status of an access request based on the provided ID and decision data." +
					"Pre-authorization is required for this action."
	)
	@ApiResponses(value = {
			@ApiResponse(
	...
	public ResponseEntity<ServiceResponse> modifyAccessRequestById(
			...
			@PathVariable("id") String id,
			@Parameter(description = "The decision data for the access request",
					example = "{\"status\": \"APPROVED\", \"message\": \"Approved for access.\", \"role\": \"ADMIN\"," +
							" \"userName\": \"john_doe\"}")
			@RequestBody AccessRequestDecision accessRequestDecision) {
	...
	public ResponseEntity<ServiceResponse> createAccessRequests(
			@Parameter(description = "The access request details to be created", required = true,
					schema = @Schema(implementation = AccessRequestDTO.class),
					example = "{\"id\": \"507f1f77bcf86cd799439011\", \"username\": \"john_doe\", \"status\": \"PENDING\"," +
	 ...
	 * @return responseEntity with data,message and status
	 */
	@Operation(summary = "Get notification count by status",
	...
	@Operation(summary = "Get notification count by status for central",
			description = "Retrieves the count of access requests with a specific status, considering central authentication.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully retrieved count",
					content = @Content(mediaType = "application/json",
							schema = @Schema(implementation = ServiceResponse.class))),
	...
	@GetMapping(value = "/{status}/notification/central",
			consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ServiceResponse> getNotificationByStatusForCentral(
			@Parameter(description = "The status to filter the access requests, with central authentication considered",
					example = "PENDING")
			@PathVariable("status") String status) {
		log.info("Getting requests count with current status {}", status);
		return ResponseEntity.status(HttpStatus.OK)
