...
@Service
@Slf4j
public class CostOfDelayServiceImpl
	...
	public KpiElement getKpiData(
						...
						(k, v) -> {
							...
							if (Filters.PROJECT == filters) {
								projectWiseLeafNodeValue(
										mapTmp, v, trendValueList, kpiElement, getRequestTrackerId(), kpiRequest);
							}
						});
		...
		List<DataCount> trendValues =
				getTrendValues(kpiRequest, kpiElement, nodeWiseKPIValue, KPICode.COST_OF_DELAY);
	 ...
	 * @return a map containing the fetched KPI data, with keys "costOfDelayData" and
	 *     "costOfDelayDataHistory"
	 */
	@Override
	public Map<String, Object> fetchKPIDataFromDb(
		...
		basicProjectConfigIds.forEach(
				basicProjectConfigId -> {
					Map<String, Object> result =
							kpiDataCacheService.fetchCostOfDelayData(
									basicProjectConfigId, KPICode.COST_OF_DELAY.getKpiId());
	...
	private void projectWiseLeafNodeValue(
		...
		projectLeafNodeList.forEach(
				node ->
						setProjectNodeValue(
								mapTmp, node, resultMap, trendValueList, requestTrackerId, excelData));
		kpiElement.setExcelData(excelData);
		kpiElement.setExcelColumns(
				KPIExcelColumn.COST_OF_DELAY.getColumns(
						projectLeafNodeList, cacheService, flterHelperService));
	}

	/**
	 * Sets the KPI value for a project node.
	 *
	 * @param mapTmp a map containing the KPI values for each node
	 * @param node the project node
	 * @param resultMap a map containing the fetched KPI data
	 * @param trendValueList a list containing the data to show on the KPI
	 * @param requestTrackerId the request tracker ID
	 * @param excelData a list containing the data to be shown in the Excel sheet
	 */
	@SuppressWarnings("unchecked")
	private void setProjectNodeValue(
			Map<String, Node> mapTmp,
			Node node,
		...
		Map<String, Double> lastNMonthMap =
				getLastNMonth(customApiConfig.getJiraXaxisMonthCount(), timeFormatMap);
		String projectName = node.getProjectFilter().getName();
		List<JiraIssue> epicList = new ArrayList<>();
		Map<String, Map<String, Integer>> howerMap = new HashMap<>();
		Map<String, List<JiraIssue>> filterWiseDataMap =
				createProjectWiseGrouping(
						(List<JiraIssue>) resultMap.get(COD_DATA), JiraIssue::getBasicProjectConfigId);
		Map<String, List<JiraIssueCustomHistory>> filterWiseHistoryDataMap =
				createProjectWiseGrouping(
						(List<JiraIssueCustomHistory>) resultMap.get(COD_DATA_HISTORY),
						JiraIssueCustomHistory::getBasicProjectConfigId);
		Map<String, List<String>> fieldMappingMap =
				(Map<String, List<String>>) resultMap.get(FIELD_MAPPING);
		List<JiraIssue> jiraIssues =
				filterWiseDataMap.get(node.getProjectFilter().getBasicProjectConfigId().toString());
		List<JiraIssueCustomHistory> jiraIssueCustomHistories =
				filterWiseHistoryDataMap.get(node.getProjectFilter().getBasicProjectConfigId().toString());
		List<String> closedStatues =
				fieldMappingMap.get(node.getProjectFilter().getBasicProjectConfigId().toString());
		...
		for (JiraIssue js : jiraIssues) {
			...
			Optional<String> epicEndDateOpt =
					jiraIssueCustomHistories.stream()
							.filter(jiraIssueCustomHistory -> jiraIssueCustomHistory.getStoryID().equals(number))
							.findFirst()
							.flatMap(
											...
											jiraIssueCustomHistory.getStatusUpdationLog().stream()
													.filter(
															jiraHistoryChangeLog ->
																	closedStatues.contains(
																			jiraHistoryChangeLog.getChangedTo().toLowerCase()))
													.findFirst()
													.map(
															jiraHistoryChangeLog ->
																	jiraHistoryChangeLog.getUpdatedOn().toString()));
			...
			if (epicEndDate != null) {
				...
				lastNMonthMap.computeIfPresent(
						date,
						(key, value) -> {
	 ...
	 * @return a map where the keys are project IDs and the values are lists of items associated with
	 *     those project IDs
	 */
	private <T> Map<String, List<T>> createProjectWiseGrouping(
			List<T> resultList, Function<T, String> projectIdExtractor) {
		return resultList.stream()
				.filter(p -> projectIdExtractor.apply(p) != null)
	...
	public Double calculateThresholdValue(FieldMapping fieldMapping) {
		return calculateThresholdValue(
				fieldMapping.getThresholdValueKPI113(), KPICode.COST_OF_DELAY.getKpiId());
	}
}
