...
public class BacklogEpicProgressServiceImpl extends JiraBacklogKPIService<Integer, List<Object>> {
			...
			if (CollectionUtils.isNotEmpty(totalIssues)
				...
				Map<String, String> epicWiseIssueSize =
						createDataCountGroupMap(
								totalIssues, jiraIssueReleaseStatus, epicIssues, fieldMapping, filterDataList);
	...
	public Map<String, Object> fetchKPIDataFromDb(
		...
		if (null != leafNode) {
			...
			List<JiraIssue> totalJiraIssue =
					jiraIssueRepository.findByBasicProjectConfigId(
			...
			final List<String> epicKeyList =
					totalJiraIssue.stream().map(JiraIssue::getEpicLinked).toList();
			Set<JiraIssue> epicJiraIssues =
					totalJiraIssue.stream()
							.filter(
									j ->
											epicKeyList.contains(j.getNumber())
	 ...
	 * @param jiraIssueReleaseStatus jiraIssueReleaseStatus
	 * @param epicIssues epicIssues
	 * @param fieldMapping fieldMapping
	 * @param iterationKpiValues iterationKpiValues
	 * @return map of epicnumber and the size of stories
	 */
	public Map<String, String> createDataCountGroupMap(
		...
		Map<String, EpicMetaData> epicIssueMap =
				epicIssues.stream()
						.collect(
								Collectors.toMap(
										JiraIssue::getNumber,
										jiraIssue ->
												new EpicMetaData(
														jiraIssue.getName(), jiraIssue.getUrl(), jiraIssue.getCreatedDate())));
		List<DataCount> dataCountList = new ArrayList<>();
		List<DataCount> openDataCountList = new ArrayList<>();
		Map<String, String> epicWiseSize = new HashMap<>();
		epicWiseJiraIssues.forEach(
				(epic, issues) -> {
					if (epicIssueMap.containsKey(epic)) {
						EpicMetaData epicMetaData = epicIssueMap.get(epic);
						DataCount statusWiseCountList =
								getStatusWiseCountList(issues, jiraIssueReleaseStatus, epicMetaData, fieldMapping);
						...
						if (!CollectionUtils.isEqualCollection(
								ReleaseKpiHelper.filterIssuesByStatus(
										issues, jiraIssueReleaseStatus.getClosedList()),
								issues)) {
	 ...
	 * @return DataCount
	 */
	DataCount getStatusWiseCountList(
			List<JiraIssue> jiraIssueList,
			JiraIssueReleaseStatus jiraIssueReleaseStatus,
			EpicMetaData epic,
			FieldMapping fieldMapping) {
		...
		Map<String, List<JiraIssue>> toDoStatusMap =
				toDoJiraIssue.stream().collect(Collectors.groupingBy(JiraIssue::getStatus));
		createIssueCountDrillDown(
				toDoStatusMap, TO_DO, toDoCount, toDoSize, issueCountDcList, fieldMapping);
		...
		Map<String, List<JiraIssue>> inProgressStatusMap =
				inProgressJiraIssue.stream().collect(Collectors.groupingBy(JiraIssue::getStatus));
		createIssueCountDrillDown(
				inProgressStatusMap,
				IN_PROGRESS,
				inProgressCount,
		...
		Map<String, List<JiraIssue>> doneStatusMap =
				doneJiraIssue.stream().collect(Collectors.groupingBy(JiraIssue::getStatus));
		createIssueCountDrillDown(
				doneStatusMap, DONE, doneCount, doneSize, issueCountDcList, fieldMapping);
	 ...
	 * create drill down
	 *
	 * @param issueCountStatusMap issueCountStatusMap
	 * @param releaseStatus releaseStatus
	 * @param releaseStatusCount releaseStatusCount
	 * @param issueSize issueSize
	 * @param issueCountDcList issueCountDcList
	 * @param fieldMapping fieldMapping
	 */
	private static void createIssueCountDrillDown(
			Map<String, List<JiraIssue>> issueCountStatusMap,
				...
				(status, issueList) ->
						drillDownList.add(
								new DataCount(
										status,
										issueList.size(),
										KpiDataHelper.calculateStoryPoints(issueList, fieldMapping),
										null)));
		DataCount releaseStatusDc =
				new DataCount(releaseStatus, releaseStatusCount, issueSize, drillDownList);
		issueCountDcList.add(releaseStatusDc);
	}

	/**
	 * sort in reverse order on the basis of created date of epics in reverse order and then on the
	 * basis of those epics whose to do and in progress issues are more should appear first
	 *
	 * @param dataCountList
	 */
		...
		if (CollectionUtils.isNotEmpty(dataCountList))
			dataCountList.sort(
					Comparator.comparing(
									(DataCount dataCount) ->
											LocalDate.parse(dataCount.getCreatedDate().split("T")[0], dateFormatter))
							.thenComparingLong(
									data ->
											// Calculate the sum of values for specified subFilters
											((List<DataCount>) data.getValue())
													.stream()
															.filter(
																	subfilter ->
		...
		if (requestTrackerId.toLowerCase().contains(KPISource.EXCEL.name().toLowerCase())
			...
			Map<String, List<JiraIssue>> epicWiseJiraIssues =
					totalIssues.stream()
							.filter(jiraIssue -> jiraIssue.getEpicLinked() != null)
							.collect(Collectors.groupingBy(JiraIssue::getEpicLinked));
			Map<String, JiraIssue> epicWiseJiraIssue =
					epicIssues.stream()
							.collect(Collectors.toMap(JiraIssue::getNumber, jiraIssue -> jiraIssue));
			KPIExcelUtility.populateEpicProgessExcelData(
					epicWiseIssueSize,
					epicWiseJiraIssue,
					excelData,
					jiraIssueReleaseStatus,
