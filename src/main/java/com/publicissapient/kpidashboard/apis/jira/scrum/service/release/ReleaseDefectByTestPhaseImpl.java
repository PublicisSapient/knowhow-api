...
/**
 * This class displays the defects that are part of the release and the phase in which the defect is
 * raised
 *
 * @author eswbogol
 */
...
public class ReleaseDefectByTestPhaseImpl extends JiraReleaseKPIService {
	 ...
	 * @param releaseNode details of project nodes
		...
		if (latestRelease != null) {
			...
			if (CollectionUtils.isNotEmpty(releaseIssues)) {
				...
				createExcelDataAndTrendValueList(
						kpiElement, filterDataList, releaseIssues, fieldMapping, excelData, latestRelease);
			}
	...
	/**
	 * @param leafNode project node details
	...
	public Map<String, Object> fetchKPIDataFromDb(
		...
		if (null != leafNode) {
			...
			List<JiraIssue> rcaExcludedIssues =
					excludeRCAMatchingIssues(leafNode, fieldMapping, releaseIssues);
	 ...
	 * @param releaseIssues releaseIssues
	 * @return excluded jiraissue
	 */
	private List<JiraIssue> excludeRCAMatchingIssues(
			Node leafNode, FieldMapping fieldMapping, List<JiraIssue> releaseIssues) {
						...
						jiraIssue -> {
							Set<String> rcas =
									projectWiseRCA.getOrDefault(
											jiraIssue.getBasicProjectConfigId(), Collections.emptySet());
							return rcas.isEmpty()
									|| jiraIssue.getRootCauseList().stream()
											.noneMatch(rc -> rcas.contains(rc.toLowerCase()));
						})
	...
	private void setDefectList(
			String defects,
			List<DataCount> defectsDataCountList,
			List<JiraIssue> defectsList,
			List<IterationKpiValue> iterationKpiValueList) {
	...
	private DataCount getStatusWiseCountList(List<JiraIssue> jiraIssueList) {
		...
		Map<String, Long> countByTestPhase =
				jiraIssueList.stream()
						.collect(
								Collectors.groupingBy(
										jiraIssue ->
												CollectionUtils.isNotEmpty(jiraIssue.getEscapedDefectGroup())
														? jiraIssue.getEscapedDefectGroup().stream()
																.findFirst()
																.orElse(UNDEFINED)
														: UNDEFINED,
										Collectors.counting()));
		Set<DataCount> dataCountList =
				countByTestPhase.entrySet().stream()
