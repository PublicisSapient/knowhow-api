...
@Component
public class ClosurePossibleTodayServiceImpl extends JiraIterationKPIService {

	public static final String UNCHECKED = "unchecked";
	private static final Logger LOGGER =
			LoggerFactory.getLogger(ClosurePossibleTodayServiceImpl.class);
			...
			if (null != dbSprintDetail) {
				...
				List<String> notCompletedIssues =
						KpiDataHelper.getIssuesIdListBasedOnTypeFromSprintDetails(
								sprintDetail, CommonConstant.NOT_COMPLETED_ISSUES);
				if (CollectionUtils.isNotEmpty(notCompletedIssues)) {
					List<JiraIssue> notCompltedJiraIssueList =
							IterationKpiHelper.getFilteredJiraIssue(notCompletedIssues, totalJiraIssueList);
					Set<JiraIssue> filtersIssuesList =
							KpiDataHelper.getFilteredJiraIssuesListBasedOnTypeFromSprintDetails(
									sprintDetail, sprintDetail.getNotCompletedIssues(), notCompltedJiraIssueList);
	...
	private void projectWiseLeafNodeValue(
			Node latestSprintNode, KpiElement kpiElement, KpiRequest kpiRequest) {
		...
		FieldMapping fieldMapping =
				configHelperService
						.getFieldMappingMap()
						.get(
								Objects.requireNonNull(latestSprintNode)
										.getProjectFilter()
		...
		if (CollectionUtils.isNotEmpty(allIssues)) {
			LOGGER.info(
					"Closure Possible Today -> request id : {} total jira Issues : {}",
					requestTrackerId,
					allIssues.size());
					...
					issue -> {
						if (issueWiseDelay.containsKey(issue.getNumber())
								&& DateUtil.stringToLocalDateTime(
												issueWiseDelay.get(issue.getNumber()).getPredictedCompletedDate(),
												DateUtil.TIME_FORMAT_WITH_SEC_DATE)
										.toLocalDate()
										.isEqual(DateUtil.getTodayDate())) {
			...
			assigneeWiseJiraIssue.forEach(
					(assignee, jiraIssues) -> {
						...
						CalculatePCDHelper.arrangeJiraIssueList(
								fieldMapping.getJiraStatusForInProgressKPI122(),
								jiraIssues,
								inProgressIssues,
								openIssues);
							...
							.filter(
									jiraIssue ->
											(jiraIssue.getAssigneeId() == null)
													...
													&& (fieldMapping
															.getJiraStatusForInProgressKPI122()
