...
public class DefectCountByServiceImpl extends JiraIterationKPIService {
		...
		if (null != leafNode) {
			...
			if (null != sprintDetails) {
				...
				if (CollectionUtils.isNotEmpty(totalIssues)) {
					...
					List<JiraIssue> totalSubTaskTaggedToSprint =
							subTaskDefects.stream()
									.filter(
											jiraIssue ->
		...
		if (latestSprint != null) {
			...
			if (fieldMapping != null) {
				...
				Map<String, IssueKpiModalValue> issueKpiModalObject =
						KpiDataHelper.createMapOfIssueModal(allCompletedDefects);
				allCompletedDefects.forEach(
						defect -> {
				...
				kpiElement.setModalHeads(
						KPIExcelColumn.DEFECT_COUNT_BY_STATUS_PIE_CHART.getColumns(
								List.of(latestSprint), cacheService, filterHelperService));
	...
	private KpiDataCategory createKpiDataCategory(
			String categoryName, String categoryValue, Integer order) {
	...
	private List<JiraIssue> filterDefects(Map<String, Object> resultMap, FieldMapping fieldMapping) {
		...
		if (CollectionUtils.isNotEmpty((List<JiraIssue>) resultMap.get(CommonConstant.TOTAL_ISSUES))) {
			return ((List<JiraIssue>) resultMap.get(CommonConstant.TOTAL_ISSUES))
					.stream()
							.filter(issue -> defectStatuses.contains(issue.getTypeName()))
