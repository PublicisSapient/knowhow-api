...
public class NetOpenTicketCountByPriorityServiceImpl
	...
	private static final Logger LOGGER =
			LoggerFactory.getLogger(NetOpenTicketCountByPriorityServiceImpl.class);
	...
	public KpiElement getKpiData(
		...
		Map<String, List<DataCount>> trendValuesMap =
				getTrendValuesMap(
						kpiRequest, kpiElement, nodeWiseKPIValue, KPICode.TICKET_COUNT_BY_PRIORITY);
	...
	public Map<String, Map<String, Map<String, Set<String>>>> fetchKPIDataFromDb(
		...
		resultListMap =
				kpiHelperService.fetchJiraCustomHistoryDataFromDbForKanban(
						leafNodeList, startDate, endDate, kpiRequest, FIELD_PRIORITY, projectWiseMapping);
		...
		return kpiHelperService.computeProjectWiseJiraHistoryByFieldAndDate(
				projectWiseNonClosedTickets, startDateForCumulative, resultListMap, FIELD_PRIORITY);
	}
					...
					if (MapUtils.isNotEmpty(jiraHistoryPriorityAndDateWiseIssueMap)) {
						...
						for (int i = 0; i < kpiRequest.getKanbanXaxisDataPoints(); i++) {
							...
							Map<String, Long> projectWisePriorityCountMap =
									filterKanbanDataBasedOnDateAndPriorityWise(
							...
							populateProjectFilterWiseDataMap(
									projectWisePriorityCountMap, trendValueMap, node, date);
	...
	private void populateProjectFilterWiseDataMap(
			Map<String, Long> projectWisePriorityMap,
		...
		projectWisePriorityMap.forEach(
				(key, value) -> {
					hoverValueMap.put(key, value.intValue());
					DataCount dcObj =
							getDataCountObject(value, projectName, date, projectNodeId, key, hoverValueMap);
	...
	public Double calculateThresholdValue(FieldMapping fieldMapping) {
		return calculateThresholdValue(
				fieldMapping.getThresholdValueKPI50(), KPICode.TICKET_COUNT_BY_PRIORITY.getKpiId());
	}
}
