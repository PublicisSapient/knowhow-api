...
/**
 * This service for managing Release BurnUp Kpi on Release Board. Gives analysis of release scope vs
 * progress for released version & release prediction based on closure rate (dev or qa) for
 * unreleased version. {@link JiraReleaseKPIService}
 */
@Slf4j
@Component
public class ReleaseBurnUpServiceImpl extends JiraReleaseKPIService {
	...
	private void dateWiseLogs(
			...
			Map<LocalDate, List<JiraIssueReferTime>> completedReleaseMap,
			Map<LocalDate, List<JiraIssueReferTime>> devCompletedReleaseMap,
		...
		allIssuesHistory.forEach(
				issueHistory -> {
											...
											if (fixVersionUpdateLog
												...
												List<JiraIssueReferTime> jiraIssueReferTime =
														createJiraIssueReferTime(
																fixVersionUpdateLog.get(lastIndex).getUpdatedOn(), jiraIssueList);
					...
					createCompletedIssuesDateWiseMap(
							issueHistory,
							completedReleaseMap,
							devCompletedReleaseMap,
							releaseIssue,
	 ...
	 */
	private void createCompletedIssuesDateWiseMap(
			JiraIssueCustomHistory issueHistory,
			Map<LocalDate, List<JiraIssueReferTime>> completedIssues,
			Map<LocalDate, List<JiraIssueReferTime>> devCompletedReleaseMap,
			List<JiraIssue> totalIssueList,
			String basicProjConfigId) {
		FieldMapping fieldMapping =
				configHelperService.getFieldMappingMap().get(new ObjectId(basicProjConfigId));
		...
		List<String> jiraReleaseDoneStatus =
				jiraIssueReleaseStatus.getClosedList().values().stream()
						.map(String::toLowerCase)
						.collect(Collectors.toList());
		List<String> devDoneStatus =
				Optional.ofNullable(fieldMapping.getJiraDevDoneStatusKPI150())
						.orElse(new ArrayList<>())
						.stream()
						.map(String::toLowerCase)
						.collect(Collectors.toList());

		List<JiraHistoryChangeLog> qaCompletionStatusLog =
				statusUpdateLog.stream()
						.filter(
								statusLog ->
										jiraReleaseDoneStatus.contains(statusLog.getChangedTo().toLowerCase())
												|| jiraReleaseDoneStatus.contains(statusLog.getChangedFrom().toLowerCase()))
						.collect(Collectors.toList());
		List<JiraHistoryChangeLog> devCompletionStatusLog =
				statusUpdateLog.stream()
						.filter(
								statusLog ->
										devDoneStatus.contains(statusLog.getChangedTo().toLowerCase())
		...
		if (CollectionUtils.isNotEmpty(qaCompletionStatusLog)) {
			final LocalDateTime updatedLog =
					getDoneDateBasedOnStatus(qaCompletionStatusLog, jiraReleaseDoneStatus, fieldMapping);
			if (updatedLog != null) {
				List<JiraIssue> jiraIssueList =
						new ArrayList<>(getRespectiveJiraIssue(totalIssueList, issueHistory));
				List<JiraIssueReferTime> jiraIssueReferTime =
						createJiraIssueReferTime(updatedLog, jiraIssueList);
				completedIssues.computeIfPresent(
		...
		if (CollectionUtils.isNotEmpty(devCompletionStatusLog)) {
			final LocalDateTime updatedLog =
					getDoneDateBasedOnStatus(devCompletionStatusLog, devDoneStatus, fieldMapping);
			if (updatedLog != null) {
				List<JiraIssue> jiraIssueList =
						new ArrayList<>(getRespectiveJiraIssue(totalIssueList, issueHistory));
				List<JiraIssueReferTime> jiraIssueReferTime =
						createJiraIssueReferTime(updatedLog, jiraIssueList);
				devCompletedReleaseMap.computeIfPresent(
						updatedLog.toLocalDate(),
						(k, v) -> {
							List<JiraIssueReferTime> jiraIssueReferTimes = new ArrayList<>(v);
							jiraIssueReferTimes.addAll(jiraIssueReferTime);
	 ...
	 *
	 * @param statusUpdateLog statusUpdateLog
	 * @param devOrQaDoneStatus Dev or Qa Done Status
	 * @param fieldMapping fieldMapping
	 * @return Map<String, LocalDate>
	 */
	private static LocalDateTime getDoneDateBasedOnStatus(
			List<JiraHistoryChangeLog> statusUpdateLog,
			List<String> devOrQaDoneStatus,
			FieldMapping fieldMapping) {
		...
		for (JiraHistoryChangeLog jiraHistoryChangeLog : statusUpdateLog) {
			...
			if (devOrQaDoneStatus.contains(jiraHistoryChangeLog.getChangedFrom().toLowerCase())
					&& jiraHistoryChangeLog
							.getChangedTo()
							.equalsIgnoreCase(fieldMapping.getStoryFirstStatus())) {
		...
		return closedStatusDateMap.values().stream()
	...
	private void releaseWiseLeafNodeValue(
		...
		String releaseState =
				Optional.ofNullable(latestRelease.getProjectHierarchy().getReleaseState()).orElse("");
		...
		Map<LocalDate, List<JiraIssueReferTime>> completedReleaseMap =
				(Map<LocalDate, List<JiraIssueReferTime>>) resultMap.get(RELEASE_PROGRESS);
		...
		Map<LocalDate, List<JiraIssueReferTime>> devCompletedIssueMap =
				(Map<LocalDate, List<JiraIssueReferTime>>) resultMap.get(DEV_COMPLETE_DATE_MAP);
		...
		if (CollectionUtils.isNotEmpty(releaseIssues)
			...
			// updated start date
			Map<String, Integer> startDateCountKPI150 =
					Optional.ofNullable(fieldMapping.getStartDateCountKPI150()).orElse(new HashMap<>());
			startLocalDate =
					Objects.requireNonNull(startLocalDate)
			...
			Boolean isPopulateByDevDone =
					ObjectUtils.defaultIfNull(fieldMapping.isPopulateByDevDoneKPI150(), false);
			Map<LocalDate, List<JiraIssueReferTime>> originalCompletedIssueMap =
					deepCopyMap(completedReleaseMap);
			Map<LocalDate, List<JiraIssueReferTime>> originalDevCompletedIssueMap =
					deepCopyMap(devCompletedIssueMap);
			Map<LocalDate, List<JiraIssueReferTime>> originalFullReleaseMap =
					deepCopyMap(fullReleaseIssueReferMap);
			...
			Map<LocalDate, List<JiraIssueReferTime>> startDateAdjustedDoneReferMap =
					Boolean.TRUE.equals(isPopulateByDevDone) ? devCompletedIssueMap : completedReleaseMap;
			Map<LocalDate, List<JiraIssueReferTime>> originalIssueDoneReferMap =
					Boolean.TRUE.equals(isPopulateByDevDone)
			...
			Map<LocalDate, List<JiraIssue>> originalIssueDoneMap =
					convertReferMapToJiraIssue(originalIssueDoneReferMap);
			fullReleaseIssueMap.forEach((k, v) -> allReleaseTaggedIssue.addAll(v));
			Map<LocalDate, List<JiraIssue>> startDateAdjustedDoneMap =
					convertReferMapToJiraIssue(startDateAdjustedDoneReferMap);

			// if no issue is closed & status is "Released" in a release prediction will not
			// be shown
			if (releaseState.equalsIgnoreCase(CommonConstant.RELEASED)
					|| MapUtils.isEmpty(startDateAdjustedDoneMap)) {
				...
				for (long i = 0; i < range && !startLocalDate.isAfter(endLocalDate); i++) {
					...
					Map<String, List<JiraIssue>> filterWiseGroupedMap =
							createFilterWiseGroupedMap(
									...
									fullReleaseIssueMap,
									overallIssues,
									startDateAdjustedDoneMap,
									overallCompletedIssues);
					overallCompletedIssues =
							filterWiseGroupedMap.getOrDefault(OVERALL_COMPLETED, new ArrayList<>());
				...
				Map<String, List<JiraIssue>> filterWiseGroupedMapTillNow =
						createFilterWiseGroupedMap(
								dateRangeTillToday,
								addedIssuesMap,
								removeIssueMap,
								fullReleaseIssueMap,
								allReleaseTaggedIssue,
								startDateAdjustedDoneMap,
								overallCompletedIssue);
				List<JiraIssue> releaseScopeToReach =
						filterWiseGroupedMapTillNow.getOrDefault(RELEASE_SCOPE, new ArrayList<>());
				List<JiraIssue> releaseProgressTillNow =
						filterWiseGroupedMapTillNow.getOrDefault(RELEASE_PROGRESS, new ArrayList<>());
				...
				final LocalDateTime predictionEndDate =
						calPredictionEndDate(
								releaseScopeToReach,
								releaseProgressTillNow,
								fieldMapping,
								avgIssueCount,
								avgStoryPoint);
				...
				// range
				Map<String, Long> durationMapPrediction =
						getDurationRangeMap(startLocalDate, predictionEndDate);
				...
				double issueSizePrediction =
						roundingOff(getStoryPoint(releaseProgressTillNow, fieldMapping));
				...
				for (long i = 0; i <= range && !startLocalDate.isAfter(predictionEndDate); i++) {
					...
					isPredictionBoundary =
							getNextPredictionDataPt(
									predictionDataMap,
									dateRange,
									isPredictionBoundary,
									releaseScopeToReach,
			...
			populateExcelDataObject(
					...
					excelData,
					releaseIssues,
					originalFullReleaseMap,
					originalCompletedIssueMap,
					originalDevCompletedIssueMap,
					fieldMapping);
			createExcelDataAndTrendValueList(
					kpiElement,
					excelData,
	 ...
	 *
	 * @param predictionDataMap Map<String, Object>
	 * @param dateRange Date Range for start & end Date
	 * @param isPredictionBoundary Flag of start of prediction
	 * @param releaseScopeToReach List<JiraIssue>
	 * @param averageDataMap Map<String, Object>
	 * @param fieldMapping fieldMapping
	 * @return prediction boundary flag
	 */
	private boolean getNextPredictionDataPt(
			Map<String, Object> predictionDataMap,
			CustomDateRange dateRange,
			boolean isPredictionBoundary,
			List<JiraIssue> releaseScopeToReach,
		...
		final boolean isStartPrediction =
				DateUtil.isWithinDateTimeRange(
								DateUtil.getTodayTime(), dateRange.getStartDateTime(), dateRange.getEndDateTime())
		...
		if (isStartPrediction && MapUtils.isNotEmpty(averageDataMap)) {
			...
			predictionDataMap.put(
					IS_ISSUE_COUNT_ACHIEVED, issueCountPrediction >= issueCountToReach || avgIssueCount == 0);
			predictionDataMap.put(
					IS_STORY_POINT_ACHIEVED, issueSizePrediction >= storyPointToReach || avgStoryPoint == 0);
			// working days between intervals namely: [days, week or month]
			long daysInterval =
					(long) CommonUtils.getWorkingDays(dateRange.getStartDate(), dateRange.getEndDate()) + 1;
			...
			predictionDataMap.put(
					ISSUE_COUNT_PREDICTION, roundingOff(Math.min(issueCountPrediction, issueCountToReach)));
			predictionDataMap.put(
					ISSUE_SIZE_PREDICTION, roundingOff(Math.min(issueSizePrediction, storyPointToReach)));
	...
	/**
	 * Based on Average Closure Rate & Release Scope to Reach cal prediction End Date
	 *
	 * @param releaseScopeToReach List<JiraIssue>
	 * @param releaseProgressTillNow List<JiraIssue>
	 * @param fieldMapping fieldMapping
	 * @param avgIssueCount avg Issue Count
	 * @param avgStoryPoint avg SP
	 * @return Prediction End Date
	 */
	private LocalDateTime calPredictionEndDate(
			List<JiraIssue> releaseScopeToReach,
			List<JiraIssue> releaseProgressTillNow,
			FieldMapping fieldMapping,
		...
		return CommonUtils.getWorkingDayAfterAdditionofDays(
				DateUtil.getTodayTime(), (int) Math.max(timeRequiredForSp, timeRequiredForIssueCount));
	}
	 ...
	 */
	private Map<String, Object> getAverageData(
			FieldMapping fieldMapping,
		...
		while (DateUtil.isWithinDateRange(currentDate, startLocalDate, LocalDate.now())) {
			completedIssuesTillTodayList.addAll(
					completedReleaseMap.getOrDefault(currentDate, new ArrayList<>()));
		...
		if (countOfDaysTillToday != 0 && CollectionUtils.isNotEmpty(completedIssuesTillTodayList)) {
			...
			avgStoryPoint =
					getStoryPoint(completedIssuesTillTodayList, fieldMapping) / countOfDaysTillToday;
			...
			releaseSpecification.setReleaseStoryPoint(
					getStoryPoint(completedIssuesTillTodayList, fieldMapping));
	 ...
	 *
	 * @param kpiElement kpiElement
	 * @param excelData List<KPIExcelData>
	 * @param iterationKpiValueList List<IterationKpiValue>
	 * @param issueCountDataGroup List<DataCountGroup>
	 * @param issueSizeCountDataGroup List<DataCountGroup>
	 * @param releaseSpecification releaseSpecification
	 */
	private void createExcelDataAndTrendValueList(
			KpiElement kpiElement,
			List<KPIExcelData> excelData,
			List<IterationKpiValue> iterationKpiValueList,
			List<DataCountGroup> issueCountDataGroup,
			List<DataCountGroup> issueSizeCountDataGroup,
			ReleaseSpecification releaseSpecification) {
		if (CollectionUtils.isNotEmpty(issueCountDataGroup)) {
			String issueCountAvgVelocity =
					AVERAGE_VELOCITY_IS
							+ releaseSpecification.getReleaseIssueCountVelocity()
							+ " ("
							+ releaseSpecification.getReleaseIssueCount()
							+ ISSUE_COUNT_IN
							+ releaseSpecification.getReleaseDuration()
							+ DAYS;
			String storyPointAvgVelocity =
					AVERAGE_VELOCITY_IS
							+ releaseSpecification.getReleaseStoryPointVelocity()
							+ " ("
							+ releaseSpecification.getReleaseStoryPoint()
							+ SPS_IN
							+ releaseSpecification.getReleaseDuration()
							+ DAYS;
	 ...
	 * @return
	 */
	public static Map<LocalDate, List<JiraIssueReferTime>> deepCopyMap(
			Map<LocalDate, List<JiraIssueReferTime>> originalMap) {
	...
	private void populateFilterWiseDataMap(
		...
		List<JiraIssue> overallIssues =
				filterWiseGroupedMap.getOrDefault(RELEASE_SCOPE, new ArrayList<>());
		List<JiraIssue> completedIssues =
				filterWiseGroupedMap.getOrDefault(RELEASE_PROGRESS, new ArrayList<>());

		double predictionIssueCount =
				(double) predictionDataMap.getOrDefault(ISSUE_COUNT_PREDICTION, 0d);
		...
		boolean isPredictionBoundary =
				(boolean) predictionDataMap.getOrDefault(IS_PREDICTION_BOUNDARY, false);
		boolean isIssueCountAchieved =
				(boolean) predictionDataMap.getOrDefault(IS_ISSUE_COUNT_ACHIEVED, false);
		boolean isStoryPointAchieved =
				(boolean) predictionDataMap.getOrDefault(IS_STORY_POINT_ACHIEVED, false);

		if (!showPrediction) {
			createDataCount(
					(long) overallIssues.size(),
					LINE_GRAPH_TYPE,
					RELEASE_SCOPE,
					issueCountDataList,
					CommonConstant.SOLID_LINE_TYPE);
			createDataCount(
					(long) completedIssues.size(),
					LINE_GRAPH_TYPE,
					RELEASE_PROGRESS,
					issueCountDataList,
					CommonConstant.SOLID_LINE_TYPE);
		} else if (isPredictionBoundary) { // populating release Progress & prediction when boundary is
			// reached
			createDataCount(
					(long) overallIssues.size(),
					LINE_GRAPH_TYPE,
		...
		if (!showPrediction) {
			createDataCount(
					getStoryPoint(overallIssues, fieldMapping),
					LINE_GRAPH_TYPE,
					RELEASE_SCOPE,
					issueSizeDataList,
					CommonConstant.SOLID_LINE_TYPE);
			createDataCount(
					getStoryPoint(completedIssues, fieldMapping),
					LINE_GRAPH_TYPE,
					RELEASE_PROGRESS,
					issueSizeDataList,
					CommonConstant.SOLID_LINE_TYPE);
	 ...
	 * @param lineCategory Line Type
	 */
	private void createDataCount(
			Object value,
			String graphType,
			String label,
			List<DataCount> issueCountDataList,
			String lineCategory) {
	 ...
	 * @param completedReleaseMap Map<LocalDate, List<JiraIssue>>
	 * @param overallCompletedIssues List<JiraIssue>
	...
	private Map<String, List<JiraIssue>> createFilterWiseGroupedMap(
			...
			List<JiraIssue> overallIssues,
			Map<LocalDate, List<JiraIssue>> completedReleaseMap,
			List<JiraIssue> overallCompletedIssues) {
		...
		List<JiraIssue> commonIssuesRemoved =
				(List<JiraIssue>) CollectionUtils.intersection(overallIssues, removedIssues);
		...
		groupedMap.put(
				OVERALL_COMPLETED, overallCompletedIssues.stream().distinct().collect(Collectors.toList()));
	 ...
	 * @param issueReleaseTagMap issueReleaseTagMap
	 * @param completedReleaseMap completedReleaseMap
	 * @param devCompletedIssueMap devCompletedIssueMap
	 * @param fieldMapping fieldMapping
	 */
	private void populateExcelDataObject(
			...
			List<KPIExcelData> excelData,
			List<JiraIssue> jiraIssueList,
			Map<LocalDate, List<JiraIssueReferTime>> issueReleaseTagMap,
			Map<LocalDate, List<JiraIssueReferTime>> completedReleaseMap,
			Map<LocalDate, List<JiraIssueReferTime>> devCompletedIssueMap,
			FieldMapping fieldMapping) {
		if (requestTrackerId.toLowerCase().contains(KPISource.EXCEL.name().toLowerCase())
			...
			final Map<String, LocalDateTime> issueWiseReleaseTagDateMap =
					getJiraIssueWiseDateMap(issueReleaseTagMap);
			final Map<String, LocalDateTime> issueWiseCompleteDateMap =
					getJiraIssueWiseDateMap(completedReleaseMap);
			final Map<String, LocalDateTime> issueWiseDevCompleteDateMap =
					getJiraIssueWiseDateMap(devCompletedIssueMap);

			KPIExcelUtility.populateReleaseBurnUpExcelData(
					jiraIssueList,
					issueWiseReleaseTagDateMap,
					issueWiseCompleteDateMap,
					issueWiseDevCompleteDateMap,
					excelData,
					fieldMapping);
		}
	}
	 ...
	 *
	 * @param dateWiseJiraIssueMap dateWiseJiraIssueMap
	 * @return Map<String, LocalDate>
	 */
	private Map<String, LocalDateTime> getJiraIssueWiseDateMap(
			Map<LocalDate, List<JiraIssueReferTime>> dateWiseJiraIssueMap) {
										...
										.map(
												issue ->
														new AbstractMap.SimpleEntry<>(
																issue.getJiraIssue().getNumber(), issue.getRefferTime())))
				.collect(
						Collectors.toMap(
	 ...
	 * @return averageDataMap
	 */
	public Map<String, Object> getAvgVelocity(
			FieldMapping fieldMapping, ReleaseSpecification releaseSpecification) {
		...
		List<JiraIssue> jiraIssues =
				jiraIssueRepository.findByBasicProjectConfigIdAndReleaseVersionsReleaseNameIn(
	 ...
	 * @param fieldMapping fieldMapping
	 * @param releaseSpecification releaseSpecification
	 * @param averageDataMap averageDataMap
	 * @param totalDurations totalDurations
