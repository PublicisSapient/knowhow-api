...
public interface AiGatewayService {
    AiProvidersResponseDTO getProviders();

    ChatGenerationResponseDTO generateChatResponse(@NotEmpty String prompt);

    void generateChatResponseAsync(
            @NotEmpty String prompt,
            Callback<ChatGenerationResponseDTO> callBack
    );
}
