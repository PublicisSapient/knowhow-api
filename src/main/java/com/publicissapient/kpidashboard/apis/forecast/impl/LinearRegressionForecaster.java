 ...
 *
 * <p>This forecaster implements simple linear regression to predict future KPI values based on
 * historical trends. It fits a line of best fit (y = mx + b) through the historical data points and
 * uses it to forecast the next value.
 *
 * <h3>Algorithm Overview:</h3>
 *
 * <ol>
 *   <li>Extracts numeric values from historical DataCount objects
 *   <li>Fits a simple linear regression model: y = slope * x + intercept
 *   <li>Uses the fitted model to predict the next value in the sequence
 *   <li>Clamps negative predictions to 0 (since most KPIs are non-negative)
 * </ol>
 *
 * <h3>When to Use:</h3>
 *
 * <ul>
 *   <li>Data shows a linear trend (increasing or decreasing)
 *   <li>Relationship between time and KPI value is approximately linear
 *   <li>Need a simple, interpretable forecasting model
 *   <li>Minimum 2 data points required (enforced by parent class)
 * </ul>
 *
 * <h3>Key Metrics:</h3>
 *
 * <ul>
 *   <li><b>Slope</b>: Rate of change (how much KPI increases/decreases per time unit)
 *   <li><b>Intercept</b>: Starting value when x=0
 *   <li><b>R²</b>: Coefficient of determination (0-1, higher = better fit)
 * </ul>
 *
 * <h3>Dependencies:</h3>
 *
 * <ul>
 *   <li>Apache Commons Math3 {@link SimpleRegression} for regression calculations
...
public class LinearRegressionForecaster extends AbstractForecastService {
	 ...
	 *
	 * <p>The regression model is of the form: <b>y = mx + b</b> where:
	 *
	 * <ul>
	 *   <li>y = predicted KPI value
	 *   <li>m = slope (rate of change)
	 *   <li>x = time index (0, 1, 2, ...)
	 *   <li>b = intercept (baseline value)
	 * </ul>
	 *
	 ...
	 *       <li>Regression model is invalid (NaN slope/intercept)
	 ...
	 * @see AbstractForecastService#createForecastDataCount(Double, String, String, String)
	 */
	@Override
	public List<DataCount> generateForecast(List<DataCount> historicalData, String kpiId) {
		...
		try {
			log.debug(
					"Starting Linear Regression forecast for KPI {} with {} data points",
					kpiId,
					values.size());
			...
			log.info(
					"Generated forecast for KPI {}: value={} [Linear Regression, y={}x+{}, R²={}]",
					kpiId,
