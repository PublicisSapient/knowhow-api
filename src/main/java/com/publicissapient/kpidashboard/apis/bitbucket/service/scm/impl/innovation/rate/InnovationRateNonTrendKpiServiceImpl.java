...
/**
 * Service implementation for calculating Innovation Rate KPI in non-trend mode. Innovation Rate
 * measures the percentage of added lines relative to total lines changed.
 */
@Component
public class InnovationRateNonTrendKpiServiceImpl
	...
	private void processToolData(
		...
		List<ScmCommits> currentCommitsList =
				filterCommitsByDateRange(commitsForBranch, currentPeriodRange);
		...
		List<ScmCommits> previousCommitsList =
				filterCommitsByDateRange(commitsForBranch, previousPeriodRange);
	...
	private double getInnovationRate(List<ScmCommits> commits) {
		double innovationRate =
				commits.stream().mapToDouble(this::calculateCommitInnovationRate).average().orElse(0.0);
		return BigDecimal.valueOf(innovationRate)
		...
		return userWiseCommits.entrySet().stream()
				.map(
						entry -> {
							...
							List<ScmCommits> currentCommitsList =
									filterCommitsByDateRange(userCommits, currentPeriodRange);
							double currentInnovationRate = getInnovationRate(currentCommitsList);

							List<ScmCommits> previousCommitsList =
									filterCommitsByDateRange(userCommits, previousPeriodRange);
							double previousInnovationRate = getInnovationRate(previousCommitsList);
							double deviationRate =
									calculateDeviationRate(currentInnovationRate, previousInnovationRate);
