...
@Component
@Slf4j
public class MeanTimeToMergeServiceImpl
		extends BitBucketKPIService<Double, List<Object>, List<MergeRequests>> {
	...
	public KpiElement getKpiData(KpiRequest kpiRequest, KpiElement kpiElement, Node projectNode)
		...
		Map<String, List<DataCount>> trendValuesMap =
				getTrendValuesMap(kpiRequest, kpiElement, nodeWiseKPIValue, KPICode.MEAN_TIME_TO_MERGE);
	...
	private void projectWiseLeafNodeValue(
			KpiElement kpiElement, Map<String, Node> mapTmp, Node projectNode, KpiRequest kpiRequest) {
		...
		List<MergeRequests> mergeRequestsList =
				fetchKPIDataFromDb(
						Arrays.asList(projectNode), localStartDate.toString(), localEndDate.toString(), null);

		// converting to map with keys collectorItemId
		Map<ObjectId, List<MergeRequests>> mergeRequestsListItemId =
				mergeRequestsList.stream()
						.collect(Collectors.groupingBy(MergeRequests::getProcessorItemId));

		List<KPIExcelData> excelData = new ArrayList<>();
		String projectName = projectNode.getProjectFilter().getName();

		ProjectFilter accountHierarchyData = projectNode.getProjectFilter();
		ObjectId configId =
		...
		reposList.forEach(
				repo -> {
					if (!CollectionUtils.isEmpty(repo.getProcessorItemList())
						...
						List<MergeRequests> mergeReqList =
								mergeRequestsListItemId.get(repo.getProcessorItemList().get(0).getId());
						if (CollectionUtils.isNotEmpty(mergeReqList)) {
							Map<String, Double> excelDataLoader = new HashMap<>();
							String repoName =
									repo.getRepositoryName() != null ? repo.getRepositoryName() : repo.getRepoSlug();
							aggMergeRequests.addAll(mergeReqList);
							List<DataCount> dataCountList =
									setWeekWiseMeanTimeToMerge(
											mergeReqList,
											repo,
											projectName,
											duration,
	...
	private List<DataCount> setWeekWiseMeanTimeToMerge(
			List<MergeRequests> mergeReqList,
			Tool tool,
			String projectName,
			String duration,
		...
		for (int i = 0; i < dataPoints; i++) {
			CustomDateRange dateRange =
					KpiHelperService.getStartAndEndDateExcludingWeekends(currentDate, duration);
			List<Double> durationList = new ArrayList<>();
			for (MergeRequests mergeReq : mergeReqList) {
				LocalDate closedDate =
						Instant.ofEpochMilli(mergeReq.getClosedDate())
			...
			Double valueForCurrentLeaf =
					ObjectUtils.defaultIfNull(AggregationUtils.average(durationList), 0.0d);
	...
	private DataCount setDataCount(String projectName, String week, Double value) {
		...
		dataCount.setData(
				String.valueOf(value == null ? 0L : TimeUnit.MILLISECONDS.toHours(value.longValue())));
	...
	@Override
	public List<MergeRequests> fetchKPIDataFromDb(
		...
		return mergeRequestRepository.findMergeRequestList(
				tools,
				new DateTime(startDate, DateTimeZone.UTC).toDate().getTime(),
				new DateTime(endDate, DateTimeZone.UTC).toDate().getTime(),
				filter);
	}
	...
	public Object calculateAggregatedValue(
		...
		if (CollectionUtils.isEmpty(node.getChildren())) {
			if (node.getValue() instanceof Integer
					|| CollectionUtils.isEmpty((Collection) node.getValue())) {
				return new ArrayList<>();
			}
	...
	private void aggregatedCodeCommitValue(
			String kpiId, List<BranchMergeReqCount> valueList, Node node) {
		...
		Map<String, List<MergeReqCount>> aggMap =
				aggregatedValueList.stream().collect(Collectors.groupingBy(MergeReqCount::getWeek));
		List<MergeReqCount> aggregatedValue = new ArrayList<>();
		aggMap.forEach(
				(key, merReq) -> {
					List<Double> value =
							merReq.stream().map(MergeReqCount::getTime).collect(Collectors.toList());
					String unit = "Hours";
					...
					if (Constant.PERCENTILE.equalsIgnoreCase(
						...
						if (null == customApiConfig.getPercentileValue()) {
							...
							aggCommit =
									new MergeReqCount(
											key,
											AggregationUtils.percentiles(value, customApiConfig.getPercentileValue()),
											unit);
							resultMap.put(
									key, AggregationUtils.percentiles(value, customApiConfig.getPercentileValue()));
						}
	...
	public Double calculateThresholdValue(FieldMapping fieldMapping) {
		return calculateThresholdValue(
				fieldMapping.getThresholdValueKPI84(), KPICode.MEAN_TIME_TO_MERGE.getKpiId());
	}
}
