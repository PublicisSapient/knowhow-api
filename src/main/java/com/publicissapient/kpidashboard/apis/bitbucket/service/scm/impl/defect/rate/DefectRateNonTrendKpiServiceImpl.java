...
/**
 * Service implementation for calculating defect rate KPI in non-trend mode. Analyzes merge requests
 * to determine the percentage of defect-related changes by identifying keywords in merge request
 * titles.
 *
 * @author KnowHOW Team
 */
@Component
public class DefectRateNonTrendKpiServiceImpl
	 ...
	 *
	 * @param kpiRequest the KPI request containing duration and data points
	 * @param mergeRequests list of merge requests to analyze
	 * @param commits list of commits (not used in this implementation)
	 * @param scmTools list of SCM tools to process
	 * @param validationDataList list to populate with validation data
	 * @param assignees set of assignees for developer name resolution
	 ...
	 * @param mergeRequests all merge requests
	 * @param assignees set of assignees
	 * @param iterationKpiValueList list to add KPI values
	 * @param validationDataList list to add validation data
	...
	private void processToolData(
		...
		CustomDateRange prevPeriodRange =
				getCustomDateRange(periodRange.getStartDateTime(), duration, dataPoints);

		List<ScmMergeRequests> currentMergedRequestsInRange =
				filterMergeRequestsByDateRange(mergeRequestsForBranch, periodRange);
		List<ScmMergeRequests> previousMergeRequestsInRange =
				filterMergeRequestsByDateRange(mergeRequestsForBranch, prevPeriodRange);
	 ...
	 *
	 * @param periodRange current period date range
	 * @param prevPeriodRange previous period date range
	 * @param userWiseMergeRequests map of user email to their merge requests
	 * @param assignees set of assignees
	 * @param tool the SCM tool
	 * @param projectName name of the project
	 * @param iterationKpiValueList list to add user-level KPI values
		...
		return userWiseMergeRequests.entrySet().stream()
				.map(
						entry -> {
							...
							List<ScmMergeRequests> currentMergedRequestsInRange =
									filterMergeRequestsByDateRange(userMergeRequests, periodRange);
							List<ScmMergeRequests> previousMergeRequestsInRange =
									filterMergeRequestsByDateRange(userMergeRequests, prevPeriodRange);

							String developerName = DeveloperKpiHelper.getDeveloperName(userEmail, assignees);

							long currentDefectMergeRequestsCount =
									countDefectMergeRequests(currentMergedRequestsInRange);
							long currentTotalMergeRequests = currentMergedRequestsInRange.size();
							double currentDefectRate =
									calculateDefectRate(currentDefectMergeRequestsCount, currentTotalMergeRequests);
							...
							return createValidationData(
									...
									null,
									currentDefectRate,
									currentDefectMergeRequestsCount,
									currentTotalMergeRequests);
						})
				.toList();
	}
	 ...
	 */
	private List<ScmMergeRequests> filterMergeRequestsByDateRange(
	 ...
	 * Calculates defect rate as a percentage.
	 *
	 * @param defectCount number of defect-related merge requests
	 * @param totalCount total number of merge requests
	 * @return defect rate percentage
	 */
	private double calculateDefectRate(long defectCount, long totalCount) {
		...
		return BigDecimal.valueOf(defectRate)
	 ...
	 * @param defectRate calculated defect rate
	 * @param defectMrCount count of defect merge requests
	 * @param mrCount total merge request count
