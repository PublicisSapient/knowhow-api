...
public class ScmCodeQualityMetricsImpl
	...
	/**
	 * Retrieves and processes KPI data for both rework rate and revert rate metrics. Uses parallel
	 * processing to fetch data from both services simultaneously for optimal performance. Combines
	 * results into unified DataCountGroup format for UI consumption.
	 *
	...
	public KpiElement getKpiData(KpiRequest kpiRequest, KpiElement kpiElement, Node projectNode)
		...
		Map<String, DataCountGroup> groupMap =
				convertMetricsToDataCountGroups(reworkMap, revertMap, dateLabel);
	...
	/**
	 * Converts rework and revert metrics maps into DataCountGroup format for UI consumption. Groups
	 * metrics by filter combinations and adds date labels.
	 *
	...
	private void processReworkData(
			Map<String, ReworkCalculation> reworkMap,
			Map<String, DataCountGroup> groupMap,
			String dateLabel) {
		for (Map.Entry<String, ReworkCalculation> entry : reworkMap.entrySet()) {
			...
			DataCount dataCount =
					createDataCount(
							"Rework Rate", entry.getValue().getPercentage(), filters[0], key, dateLabel);

			group.getValue().add(dataCount);
		}
	}

	private void processRevertData(
			Map<String, MetricsHolder> revertMap,
			Map<String, DataCountGroup> groupMap,
			String dateLabel) {
		for (Map.Entry<String, MetricsHolder> entry : revertMap.entrySet()) {
			...
			DataCount dataCount =
					createDataCount(
							"Revert Rate",
							entry.getValue().calculateRevertPercentage(),
							filters[0],
							key,
							dateLabel);
