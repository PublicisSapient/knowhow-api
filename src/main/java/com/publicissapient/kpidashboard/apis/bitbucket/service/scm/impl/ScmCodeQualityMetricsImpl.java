 ...
 * <h3>PURPOSE</h3>
 *
 * Aggregates and combines multiple SCM code quality metrics (Rework Rate and Revert Rate) into a
 * unified response for comprehensive code quality analysis.
 *
 * <h3>METRICS INCLUDED</h3>
 *
 * - <b>Rework Rate:</b> Percentage of code changes that modify recently changed lines (within 21
 * days) - <b>Revert Rate:</b> Percentage of commits that are later reverted, indicating unstable
 * changes
 *
 * <h3>OUTPUT FORMAT</h3>
 *
 * Returns DataCountGroup list containing: - filter1: "develop -> knowhow-common -> KnowHOW" (branch
 * -> repository -> project) - filter2: "Overall" or specific developer name - value: Array of data
 * objects with: - data: "Rework Rate" or "Revert Rate" - date: "10-Nov-2025 to 16-Nov-2025" -
 * kpiGroup: "develop -> knowhow-common -> KnowHOW#Overall" - value: percentage (0.0 to 100.0) -
...
public class ScmCodeQualityMetricsImpl
	...
	/**
	 * Retrieves and processes KPI data for both rework rate and revert rate metrics. Sequentially
	 * fetches data from both services and combines results into unified DataCountGroup format for UI
	 * consumption.
	 *
	...
	public KpiElement getKpiData(KpiRequest kpiRequest, KpiElement kpiElement, Node projectNode)
		...
		Map<String, DataCountGroup> groupMap =
				convertMetricsToDataCountGroups(metricsHolderMap, dateLabel);
	...
	/**
	 * Converts metrics holder map into DataCountGroup format for UI consumption. Groups metrics by
	 * filter combinations and adds date labels for both rework and revert rates.
	 *
	...
	private Map<String, DataCountGroup> convertMetricsToDataCountGroups(
			Map<String, MetricsHolder> metricsHolderMap, String dateLabel) {
		...
		for (Map.Entry<String, MetricsHolder> entry : metricsHolderMap.entrySet()) {
			...
			DataCount reworkDataCount =
					createDataCount("Rework Rate", holder.getReworkPercentage(), filters[0], key, dateLabel);
			group.getValue().add(reworkDataCount);

			// Add revert rate data
			DataCount revertDataCount =
					createDataCount(
							"Revert Rate", holder.calculateRevertPercentage(), filters[0], key, dateLabel);
	...
	private DataCount createDataCount(
			String dataType, double value, String filter1, String key, String dateLabel) {
	...
	/**
	 * Calculates KPI metrics from provided data map. Currently not implemented for this composite
	 * service.
	 *
	...
	/**
	 * Fetches KPI data from database. Not implemented as this composite service delegates to
	 * individual services.
	 *
