...
public class PushDataValidationServiceImpl {
		...
		if (MapUtils.isNotEmpty(validations)) {
			...
			validations.forEach(
					(key, value) -> {
						...
						value.forEach(
								validate -> {
									boolean lasttimeCheck =
											parameterWiseError(errors, parameter, parameterValue, validate);
									if (!timeCheck.get()) {
	...
	private void checkRequiredTimeDetails(
		...
		if (timeCheck
				&& !(errors.containsKey(START_TIME)
						&& errors.containsKey(END_TIME)
						&& errors.containsKey(DURATION))
				&& (leftKey.contains(START_TIME)
						&& leftKey.contains(END_TIME)
						&& leftKey.contains(DURATION))) {
	...
	private boolean parameterWiseError(
			Map<String, String> errors,
			String parameter,
