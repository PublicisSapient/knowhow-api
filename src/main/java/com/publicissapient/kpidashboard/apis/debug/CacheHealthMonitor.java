...
public class CacheHealthMonitor {
	...
	public void monitorCacheHealth() {
		try {
			...
			if (accountCache != null) {
				...
				logger.info(
						"[CACHE-HEALTH] accountHierarchy cache - Empty: {}, Item Count: {}",
						cacheEmpty,
						actualCount);

				if (cacheEmpty && !accountHierarchyCacheWasEmpty) {
					logger.error(
							"[CACHE-ALERT] accountHierarchy cache is EMPTY! This will cause /filterdata API to return null response");
					...
					logger.info(
							"[CACHE-RECOVERY] accountHierarchy cache has data again - Item count: {}",
							actualCount);
	...
	private int getCacheItemCount(org.springframework.cache.Cache cache) {
		try {
			...
			if (nativeCache instanceof org.ehcache.Cache) {
				...
				logger.debug(
						"[CACHE-COUNT] Native cache is not an Ehcache instance: {}",
	...
	private boolean isCacheEmpty(org.springframework.cache.Cache cache) {
		try {
			...
			if (nativeCache instanceof org.ehcache.Cache) {
				...
				if (isEmpty) {
					logger.warn(
							"[CACHE-TEST] accountHierarchy cache test failed - cache appears empty or non-functional");
				}
				return isEmpty;
			}
		} catch (Exception e) {
				...
				if (name.toString().contains("accountHierarchy")) {
					try {
						...
						if (evictions != null && evictions > lastEvictionCount) {
							logger.warn(
									"[EVICTION-ALERT] accountHierarchy cache evictions increased from {} to {}",
									lastEvictionCount,
									evictions);
	...
	public void logMemoryStatus() {
		try {
			...
			if (usagePercent > 75) {
				logger.warn(
						"[MEMORY-ALERT] High memory usage: {}MB/{}MB ({}%) - Risk of cache evictions!",
						usedMB, maxMB, String.format("%.1f", usagePercent));
			} else {
				logger.debug(
						"[MEMORY-STATUS] Memory usage: {}MB/{}MB ({}%)",
						usedMB, maxMB, String.format("%.1f", usagePercent));
			}
	...
	private void logDetailedCacheInfo() {
		try {
			...
			logger.info(
					"[DETAILED-CACHE-INFO] JVM Memory - Total: {}MB, Free: {}MB, Used: {}MB, Max: {}MB",
			...
			logger.info(
					"[DETAILED-CACHE-INFO] CacheManager class: {}", cacheManager.getClass().getSimpleName());
				...
				for (ObjectName name : cacheNames) {
					if (name.toString().contains("accountHierarchy")) {
						try {
							...
							logger.info(
									"[DETAILED-CACHE-INFO] {} - Hits: {}, Misses: {}, Evictions: {}, Puts: {}",
									name,
									hits,
									misses,
									evictions,
							...
							logger.debug(
									"[DETAILED-CACHE-INFO] Could not get MBean stats for {}: {}",
									name,
									e.getMessage());
						}
					}
			...
			logger.error(
					"[DETAILED-CACHE-INFO-ERROR] Error logging detailed cache info: {}", e.getMessage());
		}
	}
}
