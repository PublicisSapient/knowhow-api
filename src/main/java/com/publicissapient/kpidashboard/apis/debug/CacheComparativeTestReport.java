...
public class CacheComparativeTestReport {
    ...
    public Map<String, Object> generateComparativeReport() {
        ...
        testRunCounter++;
        String testId = "TEST_RUN_" + testRunCounter + "_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
    ...
    private void generateDetailedReport(String testId, TestMetrics metrics) {
        ...
        logger.info("   └─ Evict Operation: {}", metrics.evictOperationSuccess ? "✅ SUCCESS" : "❌ FAILED");
        ...
        if (metrics.cacheHits >= 0) {
            ...
            logger.info("   ├─ Cache Evictions: {} {}", metrics.cacheEvictions, 
        ...
        if (metrics.hasProblems()) {
            ...
            if (metrics.memoryUsagePercent > 75) {
                logger.error("   ❌ HIGH memory pressure ({:.2f}%) - Risk of cache evictions", metrics.memoryUsagePercent);
            }
            if (metrics.cacheEvictions > 0) {
                logger.error("   ❌ Cache evictions detected ({}) - Data being removed due to memory pressure", metrics.cacheEvictions);
            }
    ...
    private void generateHistoricalComparison() {
        ...
        if (testIds.length >= 2) {
            ...
            logger.info("🔄 COMPARING: {} vs {}", testIds[testIds.length - 2], testIds[testIds.length - 1]);
            
            // Memory Comparison
            double memoryChange = latest.memoryUsagePercent - previous.memoryUsagePercent;
            logger.info("💾 Memory Usage: {:.2f}% → {:.2f}% ({}{}%)", 
                       previous.memoryUsagePercent, latest.memoryUsagePercent,
                       memoryChange >= 0 ? "+" : "", memoryChange);
            
            // Cache Count Comparison
            int countChange = latest.cacheItemCount - previous.cacheItemCount;
            logger.info("🗄️  Cache Items: {} → {} ({}{})", 
                       previous.cacheItemCount, latest.cacheItemCount,
                       countChange >= 0 ? "+" : "", countChange);
            
            // Eviction Comparison
            long evictionChange = latest.cacheEvictions - previous.cacheEvictions;
            if (evictionChange > 0) {
                logger.error("⚠️  Cache Evictions INCREASED: {} → {} (+{})", 
                           previous.cacheEvictions, latest.cacheEvictions, evictionChange);
            } else if (evictionChange < 0) {
                logger.info("✅ Cache Evictions DECREASED: {} → {} ({})", 
                           previous.cacheEvictions, latest.cacheEvictions, evictionChange);
            } else {
            ...
            if (!previous.environmentType.equals(latest.environmentType)) {
                logger.info("🏗️  Environment Classification CHANGED: {} → {}", 
                           previous.environmentType, latest.environmentType);
            }
        }
    ...
    private void analyzeTrends() {
        ...
        logger.info("📊 Cache Empty Rate: {:.1f}% ({}/{} runs)", emptyPercentage, emptyCount, allMetrics.length);
        logger.info("🔄 Total Evictions Across All Runs: {}", totalEvictions);
        
        // Stability assessment
        if (emptyPercentage > 50) {
            logger.error("🚨 STABILITY ISSUE: Cache empty in {:.1f}% of test runs - CONFIGURATION PROBLEM!", emptyPercentage);
    ...
    private void generateRecommendations(TestMetrics metrics) {
        ...
        if (metrics.cacheEmpty && metrics.memoryUsagePercent > 75) {
            ...
            logger.error("   1. ❌ Cache is empty due to HIGH memory pressure ({:.2f}%)", metrics.memoryUsagePercent);
            logger.error("   2. 🔄 IMMEDIATE FIX: Switch from MB to entries-based configuration");
            logger.error("   3. 📝 Change: <heap unit=\"MB\">25</heap> → <heap unit=\"entries\">10000</heap>");
            ...
            logger.warn("   1. ❓ Cache is empty but memory usage is normal ({:.2f}%)", metrics.memoryUsagePercent);
            ...
            logger.warn("   1. 📈 High memory usage ({:.2f}%) - Risk of future cache evictions", metrics.memoryUsagePercent);
                ...
                if (name.toString().contains("accountHierarchy")) {
                    try {
                        ...
                        if (metrics.cacheHits + metrics.cacheMisses > 0) {
                            metrics.hitRatio = (double) metrics.cacheHits / (metrics.cacheHits + metrics.cacheMisses);
                        }
                        break;
                    } catch (Exception e) {
    ...
    private static class TestMetrics {
        ...
        public boolean hasProblems() {
            return cacheEmpty || memoryUsagePercent > 75 || cacheEvictions > 0 || 
                   !putOperationSuccess || !getOperationSuccess || hasErrors;
        }
