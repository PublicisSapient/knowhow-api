...
public enum FieldMappingEnum {
	...
	KPI40(
			...
			KPISource.JIRA.name(),
			Arrays.asList(
					"jiraStoryIdentificationKpi40", "thresholdValueKPI40", "jiraStoryCategoryKpi40")),
	...
	KPI5(
			...
			KPISource.JIRA.name(),
			Arrays.asList(
					"jiraIterationIssuetypeKpi5", "jiraIterationCompletionStatusKpi5", "thresholdValueKPI5")),
	...
	KPI135(
			"First Time Pass Rate (Iteration)",
			KPISource.JIRA.name(),
			Arrays.asList(
	...
	KPI37(
			...
			Arrays.asList(
					...
					"jiraDodKPI37",
					"thresholdValueKPI37",
					"defectRejectionLabelsKPI37")),

	KPI28(
			"Defect Count By Priority (Scrum)",
			KPISource.JIRA.name(),
			Arrays.asList(
	...
	KPI137(
			"Defect Reopen Rate (Backlog)",
			KPISource.JIRA.name(),
			Arrays.asList("jiraDefectClosedStatusKPI137")),
	...
	KPI152(
			...
			Arrays.asList(
					...
					"jiraIssueTypeNamesKPI152")),

	KPI153(
			"PI Predictability",
			KPISource.JIRA.name(),
			Arrays.asList(
					"jiraIssueEpicTypeKPI153",
	...
	KPI164(
			"Scope Churn",
			KPISource.JIRA.name(),
			Arrays.asList("jiraStoryIdentificationKPI164", "thresholdValueKPI164")),
	...
	KPI163(
			...
			Arrays.asList(
					"jiraDodKPI163",
					"excludeRCAFromKPI163",
					"testingPhaseDefectCustomField",
					"testingPhaseDefectsIdentifier",
					"testingPhaseDefectValue",
					"testingPhaseDefectComponentValue")),
	...
	KPI150(
			...
			Arrays.asList(
					...
					"populateByDevDoneKPI150",
					"jiraDevDoneStatusKPI150",
					"releaseListKPI150")),
	...
	KPI11(
			...
			Arrays.asList("thresholdValueKPI11")),

	KPI157(
			"Check-Ins & Merge Requests (Developer) ",
			KPISource.BITBUCKET.name(),
			Arrays.asList("thresholdValueKPI157")),
	...
	KPI160(
			...
			KPISource.BITBUCKET.name(),
			Arrays.asList("thresholdValueKPI160")),
	...
	KPI166(
			...
			Arrays.asList(
					...
					"jiraDodKPI166",
					"jiraProductionIncidentIdentification",
	...
	KPI170(
			...
			KPISource.JIRA.name(),
			Arrays.asList(
					"jiraIssueWaitStateKPI170", "jiraIssueClosedStateKPI170", "thresholdValueKPI170")),
	...
	KPI171(
			...
			Arrays.asList(
					...
					"jiraDorKPI171",
					"jiraDodKPI171",
					"jiraLiveStatusKPI171",
					"storyFirstStatusKPI171")),
	KPI162("PR Size", KPISource.BITBUCKET.name(), Collections.singletonList("thresholdValueKPI162")),

	KPI73(
			"Release Frequency", KPISource.JIRA.name(), Collections.singletonList("thresholdValueKPI73")),
	KPI149(
			"Happiness Index", KPISource.JIRA.name(), Collections.singletonList("thresholdValueKPI149")),
	KPI113(
			"Value delivered (Cost of Delay)",
			KPISource.JIRA.name(),
			Arrays.asList(
					"thresholdValueKPI113",
					"issueTypesToConsiderKpi113",
					"closedIssueStatusToConsiderKpi113")),
	KPI70(
			"Test Execution and pass percentage",
			KPISource.ZEPHYR.name(),
			Collections.singletonList("thresholdValueKPI70")),
	KPI8(
			"Code Build Time", KPISource.JENKINS.name(), Collections.singletonList("thresholdValueKPI8")),
	KPI172(
			"Build FREQUENCY",
			KPISource.JENKINS.name(),
			Collections.singletonList("thresholdValueKPI172")),
	KPI116(
			"Change Failure Rate",
			KPISource.JENKINS.name(),
			Collections.singletonList("thresholdValueKPI116")),
	KPI118(
			"Deployment Frequency",
			KPISource.JENKINS.name(),
			Collections.singletonList("thresholdValueKPI118")),
	KPI144(
			"Defect Count By Priority",
			KPISource.JIRA.name(),
			Collections.singletonList("jiraDodKPI144")),
	KPI143(
			"Defect Count By Assignee",
			KPISource.JIRA.name(),
			Collections.singletonList("jiraDodKPI143")),
	KPI142("Defect Count By RCA", KPISource.JIRA.name(), Collections.singletonList("jiraDodKPI142")),
	KPI173(
			"Rework Rate", KPISource.BITBUCKET.name(), Collections.singletonList("thresholdValueKPI173")),
	KPI176(
	...
	KPI182(
			"PR Success Rate",
			KPISource.BITBUCKET.name(),
			Collections.singletonList("thresholdValueKPI182")),
	KPI180(
			"Revert Rate", KPISource.BITBUCKET.name(), Collections.singletonList("thresholdValueKPI180")),
	KPI181(
			"PR Decline Rate",
			KPISource.BITBUCKET.name(),
			Collections.singletonList("thresholdValueKPI181")),

	KPI187(
			"Late Refinement",
			KPISource.JIRA.name(),
			Arrays.asList("jiraIssueTypeNamesKPI187", "jiraStatusKPI187")),
	KPI188(
			"Next Sprint Late Refinement",
			KPISource.JIRA.name(),
			Arrays.asList(
					"jiraRefinementCriteriaKPI188",
					"jiraRefinementByCustomFieldKPI188",
					"jiraRefinementMinLengthKPI188",
					"jiraRefinementKeywordsKPI188",
					"jiraIssueTypeNamesKPI188")),

	/** Kanban fieldMapping Enum starts * */
	KPI48(
			"Net Open Ticket Count By Status",
	...
	KPI49(
			...
			KPISource.JIRAKANBAN.name(),
			Arrays.asList(
					"jiraTicketVelocityIssueTypeKPI49", "ticketDeliveredStatusKPI49", "thresholdValueKPI49")),
	...
	KPI53(
			...
			Arrays.asList(
					...
					"kanbanCycleTimeIssueTypeKPI53",
					"jiraTicketTriagedStatusKPI53")),

	KPI54(
			"Ticket Open Vs Close By Priority",
			KPISource.JIRAKANBAN.name(),
			Arrays.asList(
	...
	KPI62(
			"Unit Test Coverage (Kanban) ",
			KPISource.SONARKANBAN.name(),
			Arrays.asList("thresholdValueKPI62")),

	KPI63(
			"Regression Automation Coverage",
			KPISource.ZEPHYRKANBAN.name(),
			Arrays.asList("thresholdValueKPI63")),

	KPI64(
			"Sonar Violations (Kanban) ",
			KPISource.SONARKANBAN.name(),
			Arrays.asList("thresholdValueKPI64")),

	KPI65(
			"Check-Ins & Merge Requests (Kanban) ",
			KPISource.BITBUCKETKANBAN.name(),
			Arrays.asList("thresholdValueKPI65")),

	KPI66("Code Build Time", KPISource.JENKINSKANBAN.name(), Arrays.asList("thresholdValueKPI66")),

	KPI67(
			"Sonar Tech Debt (Kanban)",
			KPISource.SONARKANBAN.name(),
			Arrays.asList("thresholdValueKPI67")),

	KPI71(
			"Test Execution and pass percentage",
			KPISource.ZEPHYRKANBAN.name(),
			Arrays.asList("thresholdValueKPI71")),
	...
	KPI159(
			...
			KPISource.BITBUCKETKANBAN.name(),
			Arrays.asList("thresholdValueKPI159")),

	KPI184(
			"Change Failure Rate", KPISource.JENKINSKANBAN.name(), Arrays.asList("thresholdValueKPI184")),

	KPI183(
			"Deployment Frequency",
			KPISource.JENKINSKANBAN.name(),
			Arrays.asList("thresholdValueKPI183")),
	...
	KPI206(
			"Flow Load",
			KPISource.JIRA.name(),
			Arrays.asList("jiraIssueTypeNamesKPI206", "jiraIssueStatusGroupByCategoryKPI206")),
	...
	KPI220(
			"Flaky Test Rate",
			KPISource.JENKINS.name(),
			List.of("e2eTestBranchKPI220", "thresholdValueKPI220")),
	KPI221("Change Failure Rate", KPISource.JENKINS.name(), List.of("thresholdValueKPI221")),
	KPI222(
	...
	KPI312(
			"Epic Hygiene",
			KPISource.JIRA.name(),
			List.of("jiraFieldsSelectionKPI312", "thresholdValueKPI312"));
