...
public class PromptGeneratorTest {
	...
	public void setUp() {
		...
		PromptDetails kpidata = new PromptDetails(PromptKeys.KPI_DATA, "Prompt for kpi details", null, Arrays.asList(
		...
		PromptDetails kpiSearch = new PromptDetails(PromptKeys.KPI_SEARCH,
				"You are a helpful assistant with a system role", "Search for relevant kpis and provide the kpiid in ",
				Arrays.asList("1. Given a list of KPIs in the format <kpiId>:<definition> : KPI_DATA and a user query"),
				"User Query: USER_QUERY", " comma-seperated string\"", Collections.singletonList("KPI_DATA"));
		...
		when(cacheService.getPromptDetails()).thenReturn(Map.of(PromptKeys.KPI_RECOMMENDATION_PROMPT,
				...
				PromptKeys.KPI_DATA, kpidata, PromptKeys.KPI_SEARCH, kpiSearch));
