...
class AIUsageServiceTest {
    ...
    @Mock
    private AIUsageStatisticsRepository aiUsageStatisticsRepository;
    ...
    void testGetAIUsageStatsForAccount_expectSuccess() {
        ...
        when(aiUsageStatisticsRepository.findTop1ByLevelNameAndStatsDateBetweenOrderByIngestTimestampDesc(anyString(), any(), any()))
                .thenReturn(new AIUsageStatistics("Account1", "acc", Instant.now(), Instant.now(), summary));

        AIUsageStatisticsResponse response = null;
        try {
        ...
        assertFalse(response.isPartialContent());
        verify(aiUsageStatisticsRepository, times(1)).findTop1ByLevelNameAndStatsDateBetweenOrderByIngestTimestampDesc(anyString(), any(), any());
    }
