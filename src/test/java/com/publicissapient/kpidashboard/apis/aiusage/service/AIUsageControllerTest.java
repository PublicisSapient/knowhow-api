...
class AIUsageControllerTest {
	...
	@InjectMocks private AIUsageController aiUsageController;
	...
	void testUploadAiUsageData_ValidPath() {
		...
		InitiateUploadResponse response =
				new InitiateUploadResponse("Success", requestId, "valid/path.csv");

		when(aiUsageService.uploadFile(anyString(), any(UUID.class), any(Instant.class)))
				.thenReturn(response);
	...
	void testUploadAiUsageData_InvalidPath() {
		...
		Instant submittedAt = Instant.now();
		InitiateUploadResponse response =
				new InitiateUploadResponse("Error", requestId, "invalid/path.csv");
	...
	void testGetProcessingStatus_InvalidId() {
		...
		when(aiUsageService.getProcessingStatus(any(UUID.class)))
				.thenThrow(new IllegalArgumentException("Invalid ID"));

		assertThrows(
				IllegalArgumentException.class, () -> aiUsageController.getProcessingStatus(requestId));
	}
}
