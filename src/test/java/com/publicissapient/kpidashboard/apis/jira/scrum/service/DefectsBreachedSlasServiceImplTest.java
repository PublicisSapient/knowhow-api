...
public class DefectsBreachedSlasServiceImplTest {
    ...
    @Test
    public void when_RequiredFieldMappingIsMissing_Expect_MetricCalculationIsNotBeingPerformed() throws ApplicationException {
        ...
        KpiElement resultedKpiElement = defectsBreachedSlasService.getKpiData(kpiRequest, kpiElement, treeAggregatorDetail);
        ...
        List<DataCountGroup> trendValuesDataCountGroupsList = (List<DataCountGroup>) resultedKpiElement.getTrendValueList();
    ...
    public void when_DataIsValid_Expect_MetricCalculationIsPerformedAccordingly() throws ApplicationException {
        ...
        setupNodes();
        //custom api config
        when(customApiConfig.getSeverity()).thenReturn(
                Map.of(
                        "s1", List.of("p1", "P1 - Blocker", "blocker", "1", "0", "p0", "Urgent", "s1", "s0"),
                        "s2", List.of("p2", "critical", "P2 - Critical", "2", "High", "s2"),
                        "s3", List.of("p3", "P3 - Major", "major", "3", "Medium", "s3"),
                        "s4", List.of("p4", "P4 - Minor", "minor", "4", "Low", "s4")
                )
        );
        ...
        s1Option.setStructuredValue(Map.of(
                "severity", "s1",
                "sla", 24.0,
                "timeUnit", "Hours"
        ));
        ...
        s3Option.setStructuredValue(Map.of(
                "severity", "s3",
                "sla", 5.0,
                "timeUnit", "Days"
        ));
        ...
        fieldMapping = new FieldMapping();
        fieldMapping.setIncludedSeveritySlasKPI195(
                List.of(s1Option, s3Option)
        );
        fieldMapping.setIncludedDefectClosureStatusesKPI195(List.of("Closed", "Resolved", "Ready for Delivery", "Done", "Ready for Sign-off"));
        Map<ObjectId, FieldMapping> basicProjectConfigIdFieldMappingMap = new HashMap<>();
        basicProjectConfigIdFieldMappingMap.put(new ObjectId(TEST_BASIC_PROJECT_CONFIG_ID), fieldMapping);
        when(configHelperService.getFieldMappingMap()).thenReturn(basicProjectConfigIdFieldMappingMap);
        ...
        when(mongoTemplate.find(any(Query.class), eq(JiraIssueCustomHistory.class))).thenReturn(generateTestJiraIssueCustomHistories());
    ...
    @Test
    public void when_KPIExcelDataIsRequestedAndDataIsValid_Expect_KpiElementContainsTheRequestedData() throws ApplicationException {
        setupTreeAggregatorDetail();
        setupNodes();
        //custom api config
        when(customApiConfig.getSeverity()).thenReturn(
    ...
    private Node createTestNode(String id, String sprintName, String startDate, String endDate) {
        ...
        ProjectFilter projectFilter =
                new ProjectFilter("project-" + id, "Project 1", new ObjectId(TEST_BASIC_PROJECT_CONFIG_ID));
    ...
    private void setupNodes() {
        ...
        ProjectFilter projectFilter =
                new ProjectFilter("project-node1", "Project1", new ObjectId(TEST_BASIC_PROJECT_CONFIG_ID));
